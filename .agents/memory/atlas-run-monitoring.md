...
Atlas runs can remain marked `running` while a provider-backed discovery source is stalled and the phase message does not change. A bounded run must be monitored for message/progress freshness, not only the active-job lock. Single-target mode is the safe way to finish one entity end-to-end: it skips discovery, registry ingestion, and global backfills while retaining the target-scoped enrichment/research journey. Cancellation must be cooperative: clearing the active slot has to prevent the worker from entering the next phase or rewriting the job as active.
