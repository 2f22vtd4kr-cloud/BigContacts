...
| 2026-08-04 | **Mobile live research console + Phase J handoff**: Replaced the mobile Reactor’s decorative-first presentation with a target-scoped public-source research console showing live target, phase, queue, operation, lanes, and evidence-aware telemetry. Added Phase J structured telemetry and a separate-worker snapshot to the Atlas status contract; web/API builds pass and the mobile screenshot is verified. The API was not restarted because job `feae7d88-3eee-48ae-99ee-7e9071801989` still owns the active slot. |
