...
| 2026-08-02 | **Controlled FAA benchmark and attribution hardening**: Buffett was rejected as an invalid celebrity benchmark. FAA bulk import completed with 5,000 real records and 0 errors. A representative 16-person, less-famous FAA aircraft-owner cohort completed cleanly at 16/16 with 0 errors: 13 social-only, 1 direct-contact candidate, and 2 with no usable contact outcome. A five-target regression confirmed organization-only and same-name handles no longer reach HNWI contact fields; review evidence remains durable. Added current-run social eligibility requiring target-person attribution plus exact fetched claim URLs, removed legacy entity-handle fallbacks before Maigret/Sherlock, added orphan-worker ownership checks and boot-time ghost-job scoping, and added regression coverage. API build passed, focused suite passed 99/99, health and Redis checks passed. This is an honest provenance/correctness milestone, not yet a 9/10 access-quality result. |
