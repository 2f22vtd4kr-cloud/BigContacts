...
| 2026-07-27 | **Import setup complete**: CI=true frozen-lockfile pnpm install (~32s), DB schema pushed (`[✓] Changes applied`), Redis + API Server (8080) + apex-finder web (23695) workflows running. `/api/healthz` → `{"status":"ok","redis":{"status":"ok","latencyMs":1}}`. SESSION_SECRET present. ENABLE_AUTO_PIPELINE=false — broad ingestion disabled. DB empty (0 entities) — cold-start did not trigger broad ingestion. Optional secrets REDIS_URL_1–5, COMPANIES_HOUSE_API_KEY, GROQ_API_KEY, OPENROUTER_API_KEY not confirmed — add for full enrichment pipeline. |
