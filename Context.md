...
| 2026-08-05 | **Gemini Deep Research lane**: Added an explicit `POST /api/research/deep-research` target-scoped job using the asynchronous Interactions API and dedicated Gemini slots `_11`–`_13`; reports and citations are returned through `/api/ingest/job/:jobId` as review-only data. The continuous Atlas scheduler does not invoke it. API build passed, the managed API workflow restarted cleanly, `/api/healthz` and `/api/system/status` returned 200 with all three dedicated slots active, and validation requests did not consume a research run. |
