...
| 2026-08-05 | **BigContacts import and environment setup verified**: Imported the BigContacts repository contents into the workspace, installed JavaScript dependencies, installed the Redis system package, applied the database schema, configured `REDIS_URL=redis://localhost:6379` and `ENABLE_AUTO_PIPELINE=false`, and started Redis, the API server, and the ApexFinder web service. Installed and verified holehe, maigret, sherlock, smolagents, and related research tooling; the optional GLiNER service remains unavailable. Final `GET /api/healthz` returned `{"status":"ok","redis":{"status":"ok","latencyMs":1}}`. |
| 2026-08-05 | **Provider secrets configured, research paused**: Securely added five permanent Redis slots, Companies House, five Groq slots, WhoXY, three dedicated Gemini Deep Research Pro Preview slots (`GEMINI_API_KEY_11`–`_13`), two Exa slots, six Tavily slots, Hugging Face, and Serper credentials. Confirmed secret existence without reading values, verified the Gemini Deep Research and Hugging Face/Serper lanes are wired into web research, and restarted only the API to load the configuration. No research was started. |
| 2026-08-05 | **Hugging Face OSINT proposal reviewed**: Declined wholesale integration of AI Sheets, OpenResearcher, WebThinker/Search-R1, and the HF Datasets cache. The existing bounded smolagents + Serper lane already covers the useful agentic-search role; TypeScript evidence schemas, exact-page validation, Redis, and durable evidence storage are safer than an LLM judge or a second cache. Any future adoption should be a benchmarked, review-only analyst export or tightly allowlisted same-domain navigation—not an admission or contact-extraction path. |
| 2026-08-05 | **Open Research provider visibility added**: `/api/system/status` now reports a separate Hugging Face + Serper capability lane with secret-presence, adapter, model, and ready/incomplete/unavailable state; it is intentionally excluded from ordinary `WEB` AI capacity. Desktop header, workspace popover, diagnostics page, and all five mobile tabs now expose the lane with responsive `OPEN OK` / `HF/S OK` indicators and tap-to-expand details. API health and live status are green; desktop/mobile web previews were checked. Existing generated-client typecheck mismatches remain unrelated to this UI change. |
