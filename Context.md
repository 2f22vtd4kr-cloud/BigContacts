...
- The previous discovery-first Atlas job `20fd5f69-8fed-40cb-a2b5-29555049025c` was safely marked failed by cold-start recovery during Phase 0 at 94/100, with 0 persisted inserts and 0 recorded target errors. Safe persona remediation then reconciled 265 entities and applied 101 deterministic findings; a duplicate cleanup dismissed 14,774 repeated pending findings with 0 errors. Provider quota/auth responses now temporarily suppress affected Tavily and direct Perplexity keys instead of retrying them for every target. Phase 0 OpenOwnership and Foundation Filing stages now emit target-scoped Reactor events. The API is rebuilt with the unified contact-state classifier, deduplicated persona suggestions, wealth/access separation, full contact-state maintenance reconciliation, and a visible duplicate-cleanup control; API/Redis/web health is green and Atlas is ready for the next sequential launch.
