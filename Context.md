...
## Current State (2026-08-04 — Atlas active; persona remediation and contact-state hardening live)
...
- The previous discovery-first Atlas job `d46eb521-c5ef-469b-9012-ed959eb41ca1` was safely marked failed by cold-start recovery during Phase 0 at 24/100, with 0 persisted inserts and 0 recorded target errors. Safe persona remediation then reconciled 265 entities and applied 101 deterministic findings; a duplicate cleanup dismissed 14,774 repeated pending findings with 0 errors. Provider quota/auth responses now temporarily suppress affected Tavily and direct Perplexity keys instead of retrying them for every target. Phase 0 OpenOwnership and Foundation Filing stages now emit target-scoped Reactor events, with numeric ownership counts. The next bounded run skips already-completed global pre-run cross-references so discovery and target-scoped enrichment can proceed without repeating the interrupted Phase 0.
...
- The prior Atlas job completed at Phase 10 after 21 source rounds, including 5 Companies House officers and one Companies House contact contribution. Exa returned usable evidence; Perplexity quota, Tavily usage limits, Gemini rate limits, and Groq rate limits constrained other provider lanes. The current Atlas retry remains active as described above.
...
- Phase J now publishes structured target/stage/tool/result telemetry and the Atlas status endpoint exposes the separate Phase J worker snapshot (pass progress, persisted records, and errors). These backend changes are built but are intentionally not live until the active Atlas worker finishes and the API can be restarted safely; the frontend fallback is live now.
...
| 2026-08-04 | **Continuous background Atlas started by explicit user request**: Set shared `ENABLE_AUTO_PIPELINE=true` and restarted the managed API workflow. Startup completed with local Redis plus all five permanent Redis slots healthy, Python OSINT tools verified, and the populated-database scheduler active. The first scheduled cycle launched Deep Web OSINT, social discovery, messenger discovery, and semantic embeddings; provider failures/rate limits remained non-fatal and no personal contact was promoted. A post-start full 11-persona sweep processed all 7 entities with 0 errors, 0 new findings, and an empty pending ledger. |
