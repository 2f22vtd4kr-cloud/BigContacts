...
## Current State (2026-08-02 — imported setup verified; pipeline idle)

### Research improvement iteration (2026-08-02):
- The imported BigContacts project is fully set up and verified. No Atlas, ingestion, enrichment, or research job was started during this setup.
- `ENABLE_AUTO_PIPELINE=false` remains set, and the active-job keys are clear. The prior FAA registry record remains completed history; no worker is currently active.
- Redis, API Server, ApexFinder web, and mockup sandbox workflows are running. ApexFinder web responds with `200`, and `/api/healthz` returns `{"status":"ok","redis":{"status":"ok","latencyMs":2}}`.
- Redis 7.2.10 is installed and responding on port 6379. PostgreSQL schema push completed successfully.
- Python 3.11 is installed in the managed workspace runtime; Holehe, Maigret, and Sherlock are installed and verified by API startup.
- All requested provider secret slots are present and remain hidden. The API is configured to use Gemini `gemini-2.0-flash-lite`, with per-key rotation, 2.5-second per-key spacing, and 1,200-token output caps to reduce quota drain.
- Full web research is serialized across entry points so one target completes before another starts. Atlas entity processing is also configured for concurrency 1; the existing provider fan-out is retained only within the current target.
- Atlas discovery now uses evidence-led prompts with explicit geography and business-role constraints. Western Europe, North America, Australia/New Zealand, and selected global wealth hubs including Singapore, Hong Kong, Japan, South Korea, Dubai, Abu Dhabi, Doha, Qatar, Bahrain, and Israel remain in scope; generic tourism, booking, listicle, and venue-topic results are excluded.
- Discovery queries include source/evidence requirements and use advanced Tavily search when available. Broad discovery admits at most one validated entity per source pass; registry and FAA rounds are capped to one record, and the admitted target is fully cooked before the next source begins.
- Dashboard entity counts and supporting metrics exclude hidden review records. Entity boolean query strings are normalized before validation, so `hidden=false` returns visible records and `hidden=true` returns quarantined records only; hide/unhide invalidates dashboard and entity-list caches.
- Latest live verification after these safeguards: visible entities `[]`, hidden entities `[]`, dashboard totals all zero, all ingestion/enrichment jobs idle, and no research started.
...
- `ENABLE_AUTO_PIPELINE=false` remains set; no Atlas run was launched during this import/setup pass.
...
| 2026-08-02 | **BigContacts import and controlled-research safeguards verified**: cloned project files were copied into the workspace, dependencies installed, Redis and Python 3.11 configured, database schema pushed, and Python OSINT tools installed. API production build passed; Redis → API → ApexFinder web workflows were restarted in order. `/api/healthz` returned Redis-healthy `200`, ApexFinder returned `200`, and all active job keys were clear. Gemini now uses Flash-Lite with per-key spacing/rotation and lower output caps; full research and Atlas entity processing are serialized at one target at a time. No research was launched. |
| 2026-08-02 | **Atlas discovery/admission and visibility correction**: restored Singapore, Hong Kong, Japan, South Korea, Dubai, Abu Dhabi, Doha, Qatar, Bahrain, and Israel as explicit discovery regions while replacing weak one-word venue prompts with geography-, role-, evidence-, and source-aware queries. Advanced Tavily search is used when available; tourism/booking/listicle noise is excluded. Broad, registry, and FAA source rounds admit at most one target before full-circle cooking. Hidden records are excluded from dashboard totals, boolean query strings are parsed correctly, and hide/unhide clears dashboard cache. API tests passed 87/87, build passed, API restarted, health returned 200, visible/hidden lists and dashboard were empty, all jobs idle, and no research started. |
