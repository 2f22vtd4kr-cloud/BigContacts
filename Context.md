...
| 2026-07-25 | GitHub import re-setup #7: CI=true pnpm install (~30s, frozen lockfile), DB schema pushed (`[✓] Changes applied`), all 4 artifacts re-registered (verifyAndReplaceArtifactToml), managed workflows auto-created (api-server: API Server, apex-finder: web, apex-mobile: expo, mockup-sandbox: Component Preview Server). Redis + API Server (8080) + apex-finder web (23695) running. `/api/healthz` → `{"status":"ok","redis":{"status":"ok","latencyMs":0}}`. Dashboard loads — 32,000 profiles visible from prior DB state; cold-start broad discovery + Western HNWI background jobs triggered. SESSION_SECRET present; REDIS_URL_1–5 (Upstash) and COMPANIES_HOUSE_API_KEY need re-adding for full enrichment and dedup persistence. |
