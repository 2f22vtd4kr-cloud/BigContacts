...
## Current State (2026-08-06 — review-only discovery candidate handoff verified)

- Provider-level `discoveryCandidates` are now carried through AI extraction and canonical web enrichment as a broad, high-recall person-discovery lane. Each candidate retains name, role, organization, basis, source URLs, social leads, and attribution status.
- Discovery candidates remain explicitly review-only: they cannot change trusted email, phone, social, access, confidence, hot, identity, attribution, or promotion fields. They are persisted separately in Atlas review metadata and feed only bounded adaptive/person follow-up.
- Organization/editorial noise is filtered before person follow-up capacity is consumed. Bounded token-window extraction supports Dutch particles such as `van der` and rejects adjacent role/title spans such as “Risk Manager” and “Portfolio Manager”.
- Validation completed: focused API tests passed 3 files / 76 tests; production API build passed; `git diff --check` passed; managed API workflow restarted; `/api/healthz` returned Redis-healthy `200`. Changes are published on `replit-sync-discovery-candidates-20260806` through PR #13; remote `main` remains unchanged.
...
| 2026-08-06 | **Review-only discovery candidate handoff**: Added provider-level `discoveryCandidates` across AI extraction, web enrichment, adaptive research, and Atlas review metadata. Candidates preserve name/role/organization/basis/source URLs/social leads/attribution status and feed bounded person follow-up without changing trusted contact, identity, access, confidence, hot, or promotion fields. Added organization/editorial and adjacent-title filtering, including Dutch `van der` name support. Focused API tests passed 76/76, API build and diff check passed, workflow restart and Redis-healthy API check passed. Published on sync branch through PR #13; remote `main` was not modified. |
