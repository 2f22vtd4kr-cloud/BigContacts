...
- Gemini Deep Research is implemented as a separate, explicit target-scoped lane through the asynchronous Interactions API. It uses only the dedicated `GEMINI_API_KEY_11`–`_13` pool, reports per-slot health as slots 11–13, persists its report/citations in the existing job poll response, and remains review-only: it never writes contacts, ownership, identity, access, or outreach state and is not part of the continuous Atlas scheduler. The active agent identifier is `deep-research-pro-preview-12-2025`; newer Google preview identifiers remain a deliberate model-version follow-up.
