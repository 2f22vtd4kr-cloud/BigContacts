...
## Current State (2026-07-31 — profile narrative cards complete; API and web running)
...
- Automatic ingestion is disabled; pipeline is idle; no Atlas, ingestion, or research job is active.
- API and web workflows were restarted after the safeguards were applied and are healthy. The web workflow was restarted again after the Intelligence Reactor redesign.
- Backend builds and frontend builds pass. Vite emits only the existing sourcemap/chunk-size warnings. API regression tests pass: 5 files / 55 tests. Full API typecheck remains blocked by the pre-existing workspace DB export mismatch and unrelated legacy errors; the production build is unaffected.
