...
/** Phase J3 identity bundles and review-only links. Identity evidence is separate from contact vectors and remains review-only until confirmed. */
export const identityBundlesTable = pgTable("identity_bundles", {
  id: serial("id").primaryKey(),
  entityId: integer("entity_id").notNull().unique().references(() => entitiesTable.id, { onDelete: "restrict" }),
...
export const identityCandidatesTable = pgTable("identity_candidates", {
  id: serial("id").primaryKey(),
  entityId: integer("entity_id").notNull().references(() => entitiesTable.id, { onDelete: "restrict" }),
  candidateEntityId: integer("candidate_entity_id").notNull().references(() => entitiesTable.id, { onDelete: "restrict" }),
  ...
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ entityPairUnique: uniqueIndex("identity_candidates_entity_pair_unique").on(table.entityId, table.candidateEntityId) }));
