...
export const researchCaseEventsTable = pgTable("research_case_events", {
  ...
  caseEventPayloadSizeCheck: check("research_case_events_payload_size_ck", sql`octet_length(${table.payload}) <= ${sql.raw(String(RESEARCH_CASE_EVENT_PAYLOAD_MAX_BYTES))}`),
}));
...
const eventPayloadSchema = z.string().refine((value) => Buffer.byteLength(value, "utf8") <= RESEARCH_CASE_EVENT_PAYLOAD_MAX_BYTES, `research case event payload must be <= ${RESEARCH_CASE_EVENT_PAYLOAD_MAX_BYTES} bytes`).refine((value) => { try { const parsed: unknown = JSON.parse(value); return parsed !== null && typeof parsed === "object" && !Array.isArray(parsed); } catch { return false; } }, "research case event payload must be a JSON object");
