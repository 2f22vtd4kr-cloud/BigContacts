...
adapter = '''import type { BureauAction, DiscoveryCaseFile, ResearchCaseFile } from "./case-bureau";
...
async function request(system: string, user: string): Promise<{ raw: string; error: string | null }> {
  const apiKey = key(); if (!apiKey) return { raw: "", error: "GEMINI_API_KEY is not configured." };
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
    ...
    try {
      ...
      if (response.ok) { try { const raw = textOf(JSON.parse(body) as GeminiResponse); if (raw) return { raw, error: null }; lastError = "Gemini Right-hand returned an empty response."; } catch { lastError = "Gemini Right-hand returned invalid JSON."; } break; }
...
export async function runGeminiRightHandDiscoveryAdvice(input: { file: DiscoveryCaseFile; iteration: number }): Promise<GeminiRightHandDiscoveryAdviceResult> {
  ...
  const parsed = extractJson(result.raw); if (!parsed) return { status: "unavailable", model: GEMINI_RIGHT_HAND_MODEL, decision: null, reason: null, focusLanes: [], confidence: null, error: "Gemini Right-hand returned invalid discovery JSON." };
...
bad = []
for path in root.rglob('*'):
    if not path.is_file() or '.git' in path.parts or path.suffix in {'.lock'}: continue
...
if bad:
    raise SystemExit('DeepSeek references remain: ' + ', '.join(bad[:20]))
