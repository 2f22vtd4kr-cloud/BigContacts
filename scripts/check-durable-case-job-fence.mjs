...
const checks = [
  ...
  ["agentic promotion requires an active target case", /apex_agentic_promotion_active_case[\s\S]*status = 'active'[\s\S]*agentic contact promotion is fenced/.test(dbSource)],
  ["stale workers cannot reactivate cancelled or lease-lost cases", /apex_research_case_cancellation_fence[\s\S]*OLD\.status = 'review'[\s\S]*NEW\.status = 'active'/.test(dbSource)],
