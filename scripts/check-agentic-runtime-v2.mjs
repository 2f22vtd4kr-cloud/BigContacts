...
assert(/const MAX_ITER = Number\.POSITIVE_INFINITY/.test(source) && !/Math\.min\(MAX_ITER, Math\.max\(1, requestedIterations\)\)/.test(source), "Investigator iterations are not artificially action-count bounded or caller-clamped");
...
for (const name of ["runHolehe", "runMaigret", "runSherlock", "runTheHarvester"]) assert(new RegExp(`${name}[\\s\\S]*?authorizeNetworkPython`).test(python), `${name} is governed by the sandbox contract`);
assert(/runOpenDeepResearch/.test(python) && /authorizePythonSandboxRequest/.test(python), "deep-research capability remains present and governed by the sandbox boundary");
