...
if (s.includes('const llm = await llmStep(prompt);') && !s.includes('const llm = await llmStep(prompt, input.investigatorLlm);')) s = s.replace('const llm = await llmStep(prompt);', 'const llm = await llmStep(prompt, input.investigatorLlm);');
...
const groqFirstPlan = /    \/\/ legacy provider fallback: avoid Gemini text-gen 429 stalls on plan steps[\s\S]*?    if \(!generated\.raw\) return unavailable\(generated\.error \?\? "Boss plan text generation returned no text\."\);/;
...
const groqFirstDiscovery = /    \/\/ legacy provider fallback for discovery brief text:[\s\S]*?    const parsed = parseBossDiscoveryResponse\(generated\.raw\);/;
...
if (pass.includes('      onLiveStep: (step) => {') && !pass.includes('void input.onInvestigationAct?.({ action: step.action')) pass = pass.replace('      onLiveStep: (step) => {', '      onLiveStep: (step) => {\n        void input.onInvestigationAct?.({ action: step.action, provider: step.provider, query: step.query, url: step.url, summary: step.summary });', 1);

// Bind selection at the Boss plan application call and next-pass execution.
