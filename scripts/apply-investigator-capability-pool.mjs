...
// 2) Selected Investigator LLM + non-LLM tools: actual investigation and promotion proposals.
// No extra Investigator decision model. DeepSeek is never an Investigator; Gemini is never an Investigator.
...
// First-class Boss-selected Investigator assignment.
...
if (s.includes('    for (const [name, fn] of providers) {') && !s.includes('const orderedProviders = selectedInvestigatorLlm')) {
  s = s.replace('    for (const [name, fn] of providers) {', '    const orderedProviders = selectedInvestigatorLlm\n      ? [...providers.filter(([name]) => name === selectedInvestigatorLlm), ...providers.filter(([name]) => name !== selectedInvestigatorLlm)]\n      : providers;\n    for (const [name, fn] of orderedProviders) {');
}
...
// Boss prompt requires the actual Investigator choice.
...
// Pass the assignment into the actual Investigator runtime.
...
if (cases.includes(nextPassCall) && !cases.includes('onInvestigationAct: async (step)')) {
  ...
  if (objectivePos >= 0) {
    cases = cases.slice(0, objectivePos)
...
console.log("Applied two-layer runtime + first-class Boss Investigator assignment + per-act living run log.");