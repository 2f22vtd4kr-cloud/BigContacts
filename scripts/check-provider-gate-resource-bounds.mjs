...
const checks = [
  ["provider waiter queue has an explicit bound", /const maxWaiters=\(\)=>boundedEnv\(\"APEX_EXTERNAL_MAX_WAITERS\"/.test(source) && /waiters\.length>=maxWaiters\(\)/.test(source)],
  ["aborted provider waiters are removed", /signal\?\.addEventListener\(\"abort\"/.test(source) && /removeWaiter\(waiter\)/.test(source)],
  ["provider state cardinality is bounded", /const maxProviderStates=\(\)=>boundedEnv\(\"APEX_EXTERNAL_MAX_PROVIDER_STATES\"/.test(source) && /providerStates\.size>=maxProviderStates\(\)/.test(source)],
  ["response cache cardinality is bounded", /const maxResponseCacheEntries=\(\)=>boundedEnv\(\"APEX_EXTERNAL_MAX_RESPONSE_CACHE_ENTRIES\"/.test(source) && /responseCache\.size>=maxResponseCacheEntries\(\)/.test(source)],
  ["response cache aggregate bytes are bounded", /const maxResponseCacheBytes=\(\)=>boundedEnv\(\"APEX_EXTERNAL_MAX_RESPONSE_CACHE_BYTES\"/.test(source) && /responseCacheBytes\+bytes>limit/.test(source)],
