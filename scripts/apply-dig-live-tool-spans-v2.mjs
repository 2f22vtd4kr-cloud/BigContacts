...
patch("artifacts/api-server/src/src/lib/agentic-web-research.ts", [
  ['import { spanFromLiveStep } from "./dig-span";', 'import { completeDigSpan, publishDigSpan, spanFromLiveStep } from "./dig-span";'],
  [
...
patch("artifacts/apex-finder/src/components/reactor-activity-only.tsx", [
  ['    .filter((activity) => activity.status === "active")', '    .filter((activity) => activity.status === "active" && activity.spanType === "tool" && Boolean(activity.tool))'],
  ['        Live telemetry only · active nodes disappear when spans retire · solid links are observed parent flow · dashed links are inferred sequence', '        Live telemetry only · active tool spans only · nodes disappear when tool spans retire · solid links are observed parent flow · dashed links are inferred sequence'],
]);

patch("scripts/check-reactor-live-integrity.mjs", [
  ['  ["graph consumes shared telemetry store", /useReactorLiveTelemetry/.test(activity)],', '  ["graph consumes shared telemetry store", /useReactorLiveTelemetry/.test(activity)],\n  ["graph renders only active tool spans", /status === "active" && activity\\.spanType === "tool" && Boolean\\(activity\\.tool\\)/.test(activity)],'],
]);
