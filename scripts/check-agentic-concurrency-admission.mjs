...
const checks = [
  ...
  ["agentic wrapper fails closed instead of queueing beyond the limit", /activeCoreRuns\s*>=\s*MAX_CONCURRENT_CORE_RUNS/.test(wrapper) && /refusing another concurrent run/.test(wrapper)],
  ["agentic wrapper releases admission on every exit path", /try\s*\{\s*return await withAgenticExecutionScope/.test(wrapper) && /finally\s*\{\s*releaseCoreRunSlot\(\);/.test(wrapper)],
