...
const checks = [
  ...
  ["embedding insertion evicts oldest entries before exceeding the ceiling", /while\(_embCache\.size >= MAX_EMBEDDING_CACHE_ENTRIES\(\)\)/.test(source) && /_embCache\.delete\(oldest\)/.test(source)],
  ["Redis hydration stops at the same ceiling", /if \(_embCache\.size >= MAX_EMBEDDING_CACHE_ENTRIES\(\)\) break/.test(source)],
