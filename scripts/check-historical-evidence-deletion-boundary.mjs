...
assert(/public\.research_evidence[\s\S]{0,900}c\.confdeltype\s*<>\s*'r'[\s\S]{0,900}ON DELETE RESTRICT/.test(migration), "explicit hardening migration must repair legacy research-evidence cascades");
assert(/public\.research_run_events[\s\S]{0,900}c\.confdeltype\s*<>\s*'r'[\s\S]{0,900}ON DELETE RESTRICT/.test(migration), "explicit hardening migration must repair legacy research-run-event cascades");
assert(/research_case_events[\s\S]{0,400}ON DELETE RESTRICT/.test(migration), "research case event ledger must remain non-destructible through case deletion");
