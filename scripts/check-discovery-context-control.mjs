...
["discovery durable projection stores structured Investigator records", /investigatorTrajectoryRecords:persistedTrajectoryRecords/.test(bureau)],
["discovery continuation preserves prior trajectory records across runs", /priorTrajectoryRecords/.test(bureau) && /persistedTrajectoryRecords=\[\.\.\.priorTrajectoryRecords/.test(bureau) && /runId:input\.runId/.test(bureau)],
["discovery continuation advances durable iteration cumulatively", /Number\(row\.iteration\?\?0\)\+Math\.max\(1,result\.iterations\)/.test(bureau)],
...
["discovery admission checks identity against observed material", /observedIdentity/.test(atlas) && /payload\.observation/.test(atlas)],
