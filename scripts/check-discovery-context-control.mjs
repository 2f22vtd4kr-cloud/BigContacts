...
["discovery admission canonicalizes source URLs before provenance matching", /function normalizeSourceUrl\(/.test(atlas) && /const normalizedSource = sourceUrl/.test(atlas) && /const normalized = normalizeSourceUrl\(String\(url\)\)/.test(atlas)],
...
["failed Investigator executions cannot become positive intelligence findings", /input\.execution === "success" \? \(input\.findings \?\? \[\]\) : \[\]/.test(fs.readFileSync("artifacts/api-server/src/src/lib/research-intelligence-engine.ts", "utf8"))],
...
["generic Investigator claims require their claimed value in observed material", /let valueObserved = false/.test(fs.readFileSync("artifacts/api-server/src/src/lib/agentic-web-research.ts", "utf8")) && /const hasValue = value\.length > 0 && observation\.includes\(value\)/.test(fs.readFileSync("artifacts/api-server/src/src/lib/agentic-web-research.ts", "utf8"))],
["discovery trajectory is accumulated rather than overwritten across continuation runs", /const accumulatedTrajectory=\[\.\.\.priorTrajectory,\.\.\.renumberedTrajectory\]/.test(bureau) && /const accumulatedRecords=\[\.\.\.priorRecords,\.\.\.offsetRecords\]/.test(bureau) && /investigatorTrajectory:accumulatedTrajectory/.test(bureau)],
["discovery continuation turns remain monotonic", /const priorIteration=Math\.max\(0,Number\(row\.iteration\?\?0\)\|\|0\)/.test(bureau) && /turn:priorIteration\+Math\.max/.test(bureau) && /const nextIteration=Math\.max\(priorIteration,priorIteration\+Math\.max/.test(bureau)],
["compatibility compaction preserves explicit state/evidence/trajectory sections", /"CURRENT STATE\\n"/.test(compactor) && /"EVIDENCE GRAPH SUMMARY\\n"/.test(compactor) && /"TRAJECTORY RECORDS\\n"/.test(compactor) && /"TRAJECTORY NOTES\\n"/.test(compactor)]
];
