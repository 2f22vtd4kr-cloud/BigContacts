...
const failures = [];
const retiredLegacyModules = new Set(["agent-orchestrator","deep-web-osint","web-osint-enricher","web-enricher","mcts-agent","research-cascade","final-target-review","web-enricher","in-house-enricher","web-osint-enricher"]);
...
function walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (unreachableCompatibilitySources.has(path.normalize(full))) continue;
...
console.log("Canonical API source has no live imports into retired legacy research modules; unreachable compatibility route bodies are separately guarded by the launch-gate topology checks.");
