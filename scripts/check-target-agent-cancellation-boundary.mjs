...
const checks = [
  ...
  ["target oversight persists immutable Investigator observation events", /persistInvestigatorObservation\(input\.caseId, input\.controlTurn, input\.act, runId\)/.test(oversight) && /actorRole: "head_investigator"/.test(oversight) && /eventType: "tool_observation"/.test(oversight)],
  ["target oversight persists immutable Boss decision events", /persistActOversight\(/.test(oversight) && /actorRole: "gemini_boss"/.test(oversight) && /eventType: "control_decision"/.test(oversight)],
  ...
  ["target oversight is anchored to the observation event", /observationEventId/.test(oversight) && /buildActEvidenceGraphs\(input\.caseId, input\.act, observationEventId, runId\)/.test(oversight)],
];
