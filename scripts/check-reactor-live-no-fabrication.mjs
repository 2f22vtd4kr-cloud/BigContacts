...
const files={bureau:path.join(root,"artifacts/apex-finder/src/components/bureau-ops-stage.tsx"),model:path.join(root,"artifacts/apex-finder/src/lib/reactor-live-model.ts"),page:path.join(root,"artifacts/apex-finder/src/pages/reactor.tsx"),hook:path.join(root,"artifacts/apex-finder/src/lib/use-bureau-live.ts")};
for(const file of Object.values(files))if(!fs.existsSync(file))throw new Error(`Missing Reactor Live source: ${file}`);
const bureauText=fs.readFileSync(files.bureau,"utf8"),modelText=fs.readFileSync(files.model,"utf8"),pageText=fs.readFileSync(files.page,"utf8"),hookText=fs.readFileSync(files.hook,"utf8");
...
const forbiddenAgePromotion=/Date\.now\(\)\s*-\s*ts\s*<\s*25_000/;
...
if(forbiddenAgePromotion.test(hookText)){console.error("FAIL  Bureau live hook promotes completed events to active solely because they are recent");process.exit(1);}
...
console.log("PASS  completed Bureau events are never promoted to active by timestamp age");
