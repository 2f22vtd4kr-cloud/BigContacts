...
const checks = [
  ...
  ["canonical discovery opens Boss before Right-hand review", /runGeminiBossDiscovery\([\s\S]*?const rightHandRaw = await import\("\.\/gemini-right-hand-reasoning"\)/.test(atlas)],
  ...
  ["canonical Atlas fails closed on invalid Right-hand oversight JSON", /rightHand\.error[\s\S]{0,400}Gemini Right-hand returned invalid oversight/.test(atlas)],
];
