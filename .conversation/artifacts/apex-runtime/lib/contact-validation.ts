...
/** Trash / non-contact values: placeholders, registrar relays, diagnostic noise. */
export function isTrashContactValue(kind: "email" | "phone" | string, value: string): boolean {
  const v = String(value || "").trim().toLowerCase();
  if (!v) return true;
  if (kind === "email") {
    if (!isValidPublicEmail(v)) return true;
    const local = v.split("@")[0] || "";
    const domain = v.split("@")[1] || "";
    if (REGISTRAR_DOMAINS.has(domain)) return true;
    if (/^(firstname|lastname|first|last|name|john|jane|doe|jdoe|flast|user|test|example|sample)$/i.test(local)) return true;
    if (/^(first\.last|firstname\.lastname|john\.doe|jane\.doe)$/i.test(local)) return true;
    if (/example\.com|test\.com|email\.com|domain\.com|sentry\.io|wixpress\.com/.test(domain)) return true;
    if (/privacy|proxy|whoisguard|registrant|redacted/.test(domain)) return true;
    return false;
  }
  if (kind === "phone") {
    const digits = v.replace(/\D/g, "");
    if (digits.length < 10 || digits.length > 15) return true;
    if (/^0+$|^1+$|^123456|^555000/.test(digits)) return true;
    return false;
  }
  return false;
}
