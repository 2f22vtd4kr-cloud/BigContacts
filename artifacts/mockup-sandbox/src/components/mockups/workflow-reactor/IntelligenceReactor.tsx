...
function fwdPath(a: NodeDef, b: NodeDef) {
  ...
  const d = Math.abs(ty - sy);
  const cp = Math.min(d * 0.55, 70);
...
// ── Main component ────────────────────────────────────────────────────────────
export function IntelligenceReactor() {
  ...
  const PW = 1600, PH = 960;

  return (
    <div style={{
      width:PW, height:PH, background:"#0b1120",
      ...
      <header style={{
        ...
        <div style={{
          ...
          fontSize:20, color: adaptive ? "#22d3ee" : "#a3e635",
      ...
      <div style={{ flex:1, position:"relative", zIndex:5, overflow:"hidden" }}>
        ...
        {/* SVG — connections */}
        <svg width={PW} height={PH - 58 - 62} style={{ position:"absolute", inset:0, zIndex:1 }}>
          <defs>
            <marker id="mLime" markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
              <path d="M0,0.5 L0,6.5 L7,3.5 z" fill="#a3e635cc" />
            </marker>
            <marker id="mCyan" markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
              <path d="M0,0.5 L0,6.5 L7,3.5 z" fill="#22d3eecc" />
            </marker>
            <marker id="mDim"  markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
          ...
          {EDGES.map(e => {
            ...
            const marker = on ? (e.adaptive ? "url(#mCyan)" : "url(#mLime)") : "url(#mDim)";
            return (
              <path key={e.id} d={d} fill="none"
                ...
                style={on ? {
                  ...
                  animation: e.adaptive
                    ? "dashBack 1s linear infinite"
                    : "dashFwd 1.2s linear infinite",
                } : {}}
              />
            );
          })}

          {/* Right-rail label for adaptive loops */}
          {adaptive && (
        ...
        {[
          { y:178, label:"REGISTRIES" },
          { y:298, label:"DISCOVERY" },
          { y:420, label:"AI  ANALYSIS" },
          { y:540, label:"SYNTHESIS" },
          { y:652, label:"CORE" },
          { y:768, label:"OUTPUT" },
        ].map(({y, label}) => (
          <div key={label} style={{
        ...
        {[236, 356, 478, 600, 716].map(y => (
          <div key={y} style={{
            position:"absolute", left:28, right:28, top:y,
            height:1, background:"linear-gradient(90deg, transparent, #192840 20%, #192840 80%, transparent)",
