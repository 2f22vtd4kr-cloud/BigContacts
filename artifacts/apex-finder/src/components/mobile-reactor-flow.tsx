...
export function MobileReactorFlow(props: MobileReactorFlowProps) {
  ...
  const statusClass = atlasState?.runStatus === "failed"
    ...
    : waitingForNextCycle
      ...
      : atlasState?.runStatus === "done"
        ? "border-[#eab308]/35 bg-[#eab308]/10 text-[#fde047]"
        : isLive
        ? "border-[#eab308]/30 bg-[#eab308]/10 text-[#fde047]"
  ...
  return (
    ...
    >
      ...
      <header
        className="sticky top-0 z-20 shrink-0 overflow-visible border-b border-[#eab308]/14 bg-[#050505]/95 px-3 py-2 backdrop-blur-md"
      ...
      >
        <div className="flex items-center gap-2">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span
                className={`h-2 w-2 shrink-0 rounded-full ${isLive ? "bg-[#eab308] shadow-[0_0_8px_#eab308]" : atlasState?.runStatus === "failed" ? "bg-rose-400" : "bg-stone-600"}`}
                style={isLive && !prefersReducedMotion() ? { animation: "pulse 2s cubic-bezier(0.4,0,0.6,1) infinite" } : undefined}
              />
              <button
                type="button"
                className={`text-[10px] font-bold uppercase tracking-[0.16em] ${isLive ? "text-[#fde047]" : "text-stone-500"}`}
                onClick={() => {
                  setShowHistory(false);
          ...
          <button
            ...
            className={`reactor-pressable flex h-10 shrink-0 items-center gap-1 rounded-lg border px-2.5 text-[10px] font-bold uppercase tracking-wider ${
              showHistory ? "border-[#eab308]/40 bg-[#eab308]/10 text-[#fde047]" : "border-[#eab308]/14 bg-[#0c0c0c] text-stone-400 hover:border-[#eab308]/35"
            }`}
          ...
          <button
            ...
            onClick={onRefresh}
            disabled={syncing}
            className="reactor-pressable flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#eab308]/14 bg-[#0c0c0c] text-stone-400 hover:border-[#eab308]/35 hover:text-[#fde047] disabled:pointer-events-none disabled:opacity-50"
        ...
        {(atlasState?.atlasTelemetry?.disposition === "contact_route_found"
          ...
          >
            ...
            {(() => {
              ...
              return (
                <a
                  ...
                  className="reactor-pressable mt-2 inline-flex items-center gap-1.5 rounded-lg border border-[#eab308]/35 bg-[#eab308]/10 px-2.5 py-1.5 text-[11px] font-semibold text-[#fef9c3] hover:border-[#facc15]/60 hover:bg-[#eab308]/15"
                >
                  Open in Profiles
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5" data-testid="reach-provenance">
              <span className="rounded-full border border-[#eab308]/25 bg-[#eab308]/10 px-1.5 py-0.5 text-[8px] font-mono font-bold uppercase tracking-wider text-[#fde047]/90">
                attributable
              </span>
              <span className="rounded-full border border-[#eab308]/12 bg-white/[0.04] px-1.5 py-0.5 text-[8px] font-mono uppercase tracking-wider text-muted-foreground">
                public surface
              </span>
              {atlasState?.atlasTelemetry?.sources != null && (
                <span className="rounded-full border border-[#eab308]/12 bg-white/[0.04] px-1.5 py-0.5 text-[8px] font-mono tabular-nums text-muted-foreground">
        ...
        {(isLive || hasReach) && (
          ...
          >
            {[
              ...
              >
                <div className={`text-[7px] font-mono uppercase tracking-wider ${chip.hot ? "text-[#fde047]/80" : "text-stone-500"}`}>
                  {chip.k}
                </div>
      ...
      {!isLive && atlasState?.runStatus === "done" && (
        ...
        >
          {[
            ...
            <div
              key={x.k}
              className="min-w-[72px] flex-1 rounded-lg border border-[#eab308]/25 bg-[#eab308]/[0.06] px-2.5 py-1.5"
            >
      ...
      {edgeHint && (
        <div
          role="status"
          className="sticky top-[52px] z-10 shrink-0 px-4 py-1.5 text-center text-[9px] font-mono uppercase tracking-wider text-[#fde047] bg-[#eab308]/15 border-b border-[#eab308]/20 backdrop-blur-sm"
      ...
      >
        <div className="mx-auto flex w-full max-w-lg flex-col gap-3 pb-8">
          {arming ? (
            <section
              className="rounded-2xl border border-[#eab308]/35 bg-[#050505] p-3 shadow-[0_0_48px_rgba(234,179,8,0.12)]"
            ...
            >
              <div className="mb-3 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="relative flex h-2 w-2">
                    <span className="absolute inline-flex h-full w-full rounded-full bg-[#eab308] opacity-60" style={{ animation: motionOrNone(`reactorShimmer ${REACTOR_SHIMMER_MS}ms ease-in-out infinite`) }} />
                    <span className="relative inline-flex h-2 w-2 rounded-full bg-yellow-300" />
                  </span>
                  <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#fde047]/90">Arming desk…</div>
                </div>
                <div className="h-1.5 w-20 overflow-hidden rounded-full bg-stone-800">
                  <div
                    className="h-full rounded-full bg-[#eab308]/70"
                    style={{
                      width: "55%",
              ...
              <div className="overflow-hidden rounded-xl border border-[#eab308]/15 bg-[#0a0a0a] shadow-[inset_0_1px_0_rgba(234,179,8,0.06)]">
                ...
                <div className="border-b border-[#eab308]/08 px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <div className="h-2.5 w-2.5 shrink-0 rounded-full bg-[#eab308]/50" />
            ...
            <section
              ...
              className={`rounded-2xl border p-3 ${
                showHistory
                  ...
                  : "border-[#eab308]/25 bg-[#050505] shadow-[0_0_48px_rgba(240,180,41,0.1)]"
              }`}
            ...
            >
              <div className="mb-3 space-y-2 px-0.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className={`text-[10px] font-bold uppercase tracking-[0.2em] ${
                      showHistory ? "text-stone-300" : "text-[#fde047]/90"
                    }`}>
                    ...
                    {!showHistory && isLive && (
                      <span className="rounded-full border border-[#eab308]/30 bg-[#eab308]/10 px-1.5 py-0.5 text-[8px] font-mono font-bold uppercase tracking-wider text-[#fde047]">
                        live
                      </span>
                    )}
                  </div>
                  <div
                    className={`text-[10px] font-mono tabular-nums transition-colors ${
                      eventCountPulse ? "text-[#fde047]" : "text-stone-500"
                    }`}
                ...
                {!showHistory && liveEvents.length > 1 && (
                  ...
                  >
                    <div
                      className="h-full rounded-full bg-[#eab308]/70 transition-[width] duration-300 ease-out"
              ...
              {showHistory && deskEvents.length > 0 && (
                <div className="mb-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-[#eab308]/12 bg-[#0a0a0a]/90 px-3 py-2 text-[12px] text-stone-200 outline-none placeholder:text-muted-foreground/60 focus-visible:border-[#eab308]/50 focus-visible:ring-1 focus-visible:ring-yellow-400/40"
                    autoComplete="off"
                    spellCheck={false}
                  />
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`reactor-pressable min-h-[32px] rounded-full border px-2.5 py-1 text-[10px] font-mono font-bold uppercase tracking-wider transition-colors ${
                          selected
                            ? "border-[#eab308]/50 bg-[#eab308]/15 text-[#fde047]"
              ...
              {showHistory && deskEvents.length > 0 && liveEvents.length === 0 && (
                <div className="mb-3 rounded-lg border border-dashed border-[#eab308]/15 bg-[#0c0c0c]/50 px-3 py-3 text-center text-[11px] text-muted-foreground" data-testid="history-filter-empty">
            ...
            <div
              className={`flex min-h-[320px] flex-col items-center justify-center rounded-2xl border px-6 text-center transition-colors duration-300 ${
                isLive
                  ? "border-[#eab308]/30 bg-[#eab308]/[0.05]"
                  : showHistory
            ...
            >
              <div className="relative mb-3">
                <Radio className={`h-8 w-8 ${isLive ? "text-yellow-400/80" : "text-muted-foreground/60"}`} />
                {isLive && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-2.5 w-2.5">
                    <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#eab308] opacity-50" />
              ...
              {(isLive || forceLiveEmpty) && (
                <div
                  className="mt-5 w-full max-w-[280px] overflow-hidden rounded-xl border border-[#eab308]/20 bg-[#0a0a0a]/90"
