...
function statusIcon(status: Job["status"]) {
  if (status === "running" || status === "queued") return <Loader2 className="w-3.5 h-3.5 animate-spin text-[#00e68a]" />;
  if (status === "done") return <CheckCircle2 className="w-3.5 h-3.5 text-[#34f5a0]" />;
  if (status === "failed") return <XCircle className="w-3.5 h-3.5 text-red-400" />;
  if (status === "cancelled") return <XCircle className="w-3.5 h-3.5 text-muted-foreground" />;
  return <div className="w-3.5 h-3.5 rounded-full border border-[#00e68a]/12 bg-muted/30" />;
}

function statusColor(status: Job["status"]) {
  if (status === "running" || status === "queued") return "text-[#00e68a]";
  if (status === "done") return "text-[#34f5a0]";
...
function categoryIcon(cat: string) {
  if (cat === "Discovery") return <Globe className="w-3.5 h-3.5 text-[#00e68a]" />;
  if (cat === "Enrichment") return <Search className="w-3.5 h-3.5 text-[#00e68a]" />;
  if (cat === "Analysis") return <Brain className="w-3.5 h-3.5 text-[#00e68a]" />;
...
function IngestorCard({ job, onTrigger }: { job: Job; onTrigger: (id: string) => void }) {
  ...
  return (
    <div className={cn("rounded-2xl border border-[#00e68a]/10 bg-card/30 overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] transition-colors", isActive && "border-emerald-400/30 bg-emerald-500/[0.04]")} data-testid={`card-job-${job.id}`}>
      <button
        onClick={() => setOpen(o => !o)}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[9px] font-mono uppercase tracking-[0.1em] text-muted-foreground/70 border border-[#00e68a]/12 bg-background/40 px-1.5 py-0.5 rounded-md hidden sm:block whitespace-nowrap">{job.category}</span>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0 ml-2">
          ...
          {job.status === "done" && job.inserted > 0 && (
            <span className="text-[10px] font-mono text-[#34f5a0] hidden sm:block">+{job.inserted.toLocaleString()}</span>
          )}
      ...
      {open && (
        <div className="border-t border-[#00e68a]/10 px-4 py-3 bg-background/30 space-y-3">
          {/* Metrics */}
          {job.status !== "idle" && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-mono">
              ...
              {job.inserted > 0 && <span className="text-[#34f5a0]">+{job.inserted.toLocaleString()} inserted</span>}
          ...
          {(isActive || job.status === "done") && (
            <div className="h-1.5 rounded-full bg-muted overflow-hidden">
              {job.status === "queued" ? (
                ...
                <div className="h-full rounded-full bg-[#00e68a] transition-all duration-500" style={{ width: "100%" }} />
              ) : (
          ...
          {job.endpoint && (
            <button
              ...
              className={cn(
                ...
                isActive
                  ? "border-[#00e68a]/12 text-muted-foreground opacity-50 cursor-not-allowed"
                  : "border-emerald-400/35 text-emerald-100 bg-emerald-400/10 hover:bg-emerald-400/15"
              )}
            >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleRun} disabled={running} data-testid="button-run-ai-analysts" className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded border font-mono text-[11px] uppercase tracking-wider transition-colors", running ? "border-[#00e68a]/12 text-muted-foreground opacity-50 cursor-not-allowed" : "border-primary/40 text-[#00e68a] bg-primary/5 hover:bg-primary/15")}>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            ...
            { label: "Pending", val: stats.pending ?? 0, color: "text-[#00e68a]" },
            { label: "Applied", val: stats.applied ?? 0, color: "text-[#34f5a0]" },
            { label: "Dismissed", val: stats.dismissed ?? 0, color: "text-muted-foreground" },
          ].map(({ label, val, color }) => (
            <div key={label} className="p-3 rounded-xl border border-[#00e68a]/12 bg-card/30 text-center">
      ...
      <div className="flex flex-wrap gap-1.5">
        <button onClick={() => setFilter("all")} className={cn("px-2 py-1 rounded text-[10px] font-mono uppercase tracking-wider border transition-colors", filter === "all" ? "border-primary/40 text-[#00e68a] bg-primary/10" : "border-[#00e68a]/12 text-muted-foreground hover:text-foreground")}>All Analysts</button>
        {personas.map(p => (
          <button key={p} onClick={() => setFilter(p)} className={cn("px-2 py-1 rounded text-[10px] font-mono uppercase tracking-wider border transition-colors", filter === p ? "border-primary/40 text-[#00e68a] bg-primary/10" : "border-[#00e68a]/12 text-muted-foreground hover:text-foreground")}>
      ...
      <div className="space-y-2">
        ...
        {filteredLogs.map((log: any) => (
          <div key={log.id} className="p-3 rounded-lg border border-[#00e68a]/12 bg-card/30" data-testid={`card-suggestion-${log.id}`}>
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2 flex-wrap min-w-0">
                <span className="text-[9px] font-mono uppercase tracking-widest text-[#00e68a] border border-primary/30 bg-primary/5 px-1.5 py-0.5 rounded">{personaLabel(log.persona)}</span>
                <span className={cn(
                  ...
                  log.priority === "MEDIUM" ? "text-[#a7f3d0] border-[#00e68a]/30 bg-[#00e68a]/10" :
                  "text-muted-foreground border-[#00e68a]/12 bg-muted/20"
                )}>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => fetch(`${BASE}/api/improve/logs/${log.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "applied" }) }).then(() => setLogs(prev => prev.map(l => l.id === log.id ? { ...l, status: "applied" } : l)))}
                  ...
                  className="text-[9px] font-mono text-[#34f5a0]/60 hover:text-[#34f5a0] border border-[#00e68a]/20 px-1.5 py-0.5 rounded transition-colors">Apply</button>
                <button onClick={() => fetch(`${BASE}/api/improve/logs/${log.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "dismissed" }) }).then(() => setLogs(prev => prev.filter(l => l.id !== log.id)))}
                  ...
                  className="text-[9px] font-mono text-muted-foreground/40 hover:text-muted-foreground border border-[#00e68a]/10 px-1.5 py-0.5 rounded transition-colors">Dismiss</button>
              </div>
            </div>
            <p className="text-[11px] font-mono text-foreground/80 mt-1.5 leading-relaxed">{log.suggestion}</p>
            {log.actionTaken && <p className="text-[10px] font-mono text-[#34f5a0]/60 mt-1 bg-[#34f5a0]/5 border border-[#00e68a]/10 rounded px-2 py-1">{log.actionTaken}</p>}
          </div>
        ))}
      </div>
    </div>
...
function DuplicatesTab() {
  ...
  if (visible.length === 0) return (
    <div className="flex flex-col items-center justify-center py-16 text-center" data-testid="empty-state-duplicates">
      <CheckCircle2 className="w-8 h-8 text-[#34f5a0] mb-3" />
  ...
  return (
    <div className="space-y-3">
      ...
      {visible.map((pair: any) => {
        ...
        return (
          <div key={key} className="border border-[#00e68a]/12 rounded-lg overflow-hidden" data-testid={`card-duplicate-${key}`}>
            <button onClick={() => setExpanded(expanded === key ? null : key)} data-testid={`button-toggle-duplicate-${key}`} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-muted/15 transition-colors text-left">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-[9px] font-mono text-[#00e68a] border border-primary/30 px-1.5 py-0.5 rounded">{Math.round((pair.similarity ?? 0) * 100)}%</span>
            ...
            {expanded === key && (
              <div className="border-t border-[#00e68a]/10 p-3 bg-background/30 space-y-3">
                <div className="grid sm:grid-cols-2 gap-2">
                  {[{ entity: primary, label: "KEEP" }, { entity: secondary, label: "MERGE INTO (PRIMARY)" }].map(({ entity, label }) => (
                    <div key={entity.id} className={cn("p-3 rounded border text-xs font-mono space-y-1", label === "KEEP" ? "border-primary/30 bg-primary/5" : "border-[#00e68a]/12 bg-card/30")}>
                      ...
                      {entity.bayesianScore != null && <div className="text-[#00e68a]">Signal: {((entity.bayesianScore ?? 0) * 100).toFixed(0)}</div>}
                    </div>
                  ))}
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <button onClick={() => setSwapped(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n; })} data-testid={`button-swap-duplicate-${key}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-[#00e68a]/12 text-muted-foreground text-[10px] font-mono uppercase hover:text-foreground hover:border-primary/40 transition-colors">
                    <GitMerge className="w-3 h-3" /> Swap Direction
                  </button>
                  <button onClick={() => merge(pair)} disabled={isMerging} data-testid={`button-merge-duplicate-${key}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-primary/40 text-[#00e68a] text-[10px] font-mono uppercase bg-primary/5 hover:bg-primary/15 transition-colors disabled:opacity-50">
                    {isMerging ? <Loader2 className="w-3 h-3 animate-spin" /> : <GitMerge className="w-3 h-3" />}
                    {isMerging ? "Merging…" : "Merge"}
                  </button>
                  <button onClick={() => dismiss(pair)} data-testid={`button-dismiss-duplicate-${key}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-[#00e68a]/12 text-muted-foreground text-[10px] font-mono uppercase hover:text-foreground transition-colors">
...
function LiveActivityTab({ jobs }: { jobs: Job[] }) {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="space-y-2">
            {active.map(job => (
              <div key={job.id} className="p-3 rounded-lg border border-primary/30 bg-primary/5 flex items-center gap-3" data-testid={`card-active-job-${job.id}`}>
                <Loader2 className="w-4 h-4 animate-spin text-[#00e68a] shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-mono font-bold text-[#00e68a] truncate">{job.label}</div>
                ...
                {job.inserted > 0 && <span className="text-[10px] font-mono text-[#34f5a0] shrink-0">+{job.inserted.toLocaleString()}</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {active.length === 0 && (
        <div
          className="rounded-2xl border border-dashed border-emerald-400/20 bg-gradient-to-br from-emerald-500/[0.05] via-card/30 to-transparent px-4 py-6"
        ...
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2 shrink-0">
              <Link
                href="/data-sources"
                className="inline-flex min-h-[40px] items-center rounded-xl border border-[#00e68a]/10 bg-background/50 px-3.5 py-2 text-[11px] font-semibold text-foreground hover:border-emerald-400/35"
              ...
              <Link
                href="/jobs#sources"
                className="inline-flex min-h-[40px] items-center rounded-xl border border-emerald-400/30 bg-emerald-400/10 px-3.5 py-2 text-[11px] font-semibold text-emerald-100 hover:bg-emerald-400/15"
      ...
      {recent.length > 0 && (
        <div>
          ...
          <div className="space-y-1">
            {recent.map(job => (
              <div key={job.id} className="flex items-center gap-3 px-3 py-2 rounded border border-[#00e68a]/10 bg-card/10">
                {statusIcon(job.status)}
                <span className="text-xs font-mono text-foreground/80 truncate flex-1">{job.label}</span>
                {job.inserted > 0 && <span className="text-[10px] font-mono text-[#34f5a0]">+{job.inserted.toLocaleString()}</span>}
      ...
      {idle.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {idle.map(job => (
              <span key={job.id} className="text-[10px] font-mono text-muted-foreground/55 border border-[#00e68a]/08 bg-card/25 px-2.5 py-1 rounded-lg">{job.label}</span>
            ))}
          </div>
        </div>
      )}
...
export default function BackgroundJobs() {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      {/* Header */}
      <div className="border-b border-[#00e68a]/12 bg-card/30 px-4 md:px-6 py-4 flex-shrink-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <Radio className="w-5 h-5 text-[#00e68a] shrink-0" aria-hidden />
            <div className="min-w-0">
          ...
          <button
            onClick={fetchJobs}
            className="flex min-h-[40px] w-full sm:w-auto items-center justify-center gap-1.5 rounded-lg border border-[#00e68a]/12 px-3 py-1.5 text-[10px] font-mono text-muted-foreground hover:text-foreground hover:border-primary/40 transition-colors shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
          >
      ...
      {apiOffline && (
        <div className="mx-4 md:mx-6 mt-3 flex items-start gap-2 rounded-lg border border-[#00e68a]/30 bg-[#00e68a]/5 px-3 py-2.5 text-[11px] text-[#a7f3d0]/90" role="status">
          <span className="font-semibold text-[#a7f3d0] shrink-0">API offline</span>
      ...
      {/* Tabs */}
      <div className="border-b border-[#00e68a]/12 bg-card/20 flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-4 md:p-6">
        ...
        {activeTab === "sources" && (
          <div className="space-y-6">
            ...
            {/* OSINT Tools link */}
            <div className="flex items-center gap-3 p-3 rounded-lg border border-[#00e68a]/10 bg-card/20">
              ...
              <Link href="/_osint-tools" className="text-[10px] font-mono text-[#00e68a]/60 hover:text-[#00e68a] whitespace-nowrap flex items-center gap-0.5">
