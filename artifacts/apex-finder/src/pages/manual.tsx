...
function CompletenessDemo() {
  ...
  return (
    <div className="space-y-2 my-4">
      {rows.map((r) => (
        <div key={r.level} className="bg-card/50 border border-[#00e68a]/10 rounded-2xl p-3.5 flex items-start gap-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
          <span
...
function Callout({ children, title, tone = "primary" }: { children: React.ReactNode; title?: string; tone?: "primary" | "warn" }) {
  const border = tone === "warn" ? "border-[#00e68a]" : "border-primary";
  const bg = tone === "warn" ? "bg-[#00e68a]/5" : "bg-primary/5";
  const titleColor = tone === "warn" ? "text-[#00e68a]" : "text-primary";
  return (
    ...
    content: (
      <>
        ...
        <ul className="space-y-3 text-sm text-muted-foreground mb-4">
          ...
          <li className="flex gap-2"><Globe className="w-4 h-4 text-[#00e68a] shrink-0 mt-0.5" /><span><strong className="text-foreground">Browser steps</strong> — opening public pages and reading what they say about the target.</span></li>
          <li className="flex gap-2"><Activity className="w-4 h-4 text-[#34f5a0] shrink-0 mt-0.5" /><span><strong className="text-foreground">Analyst steps</strong> — writing down only contacts that can be proven.</span></li>
          <li className="flex gap-2"><Shield className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" /><span><strong className="text-foreground">Footprint steps</strong> — checking whether the same name shows up on other public sites.</span></li>
          <li className="flex gap-2"><Database className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" /><span><strong className="text-foreground">Domain steps</strong> — checking who registered the website and when.</span></li>
        </ul>
      ...
      <>
        ...
        <Callout title="Org vs Personal" tone="warn">
          ...
          <ul className="list-disc pl-5 space-y-1 mt-2 text-muted-foreground">
            <li><span className="text-[#a7f3d0]">Emerald</span> — personal or role contact (what outreach wants)</li>
            <li><span className="text-emerald-300">Violet</span> — organization inbox (kept, never labeled Personal)</li>
            <li><span className="text-stone-300">Sky</span> — social handles</li>
            <li><span className="text-[#a7f3d0]">Amber</span> — related people from ownership resolution</li>
          </ul>
    ...
    content: (
      <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] font-mono text-muted-foreground mb-4">
          <div className="border border-[#00e68a]/12 rounded p-2">AI pools (header) — Groq, Perplexity, Gemini, Tavily, Exa</div>
          <div className="border border-[#00e68a]/12 rounded p-2">Open research — Hugging Face, Serper, Mistral, Scrapfly, Zenrows</div>
          <div className="border border-[#00e68a]/12 rounded p-2">Bureau — Gemini Boss, NVIDIA NIM advisor</div>
          <div className="border border-[#00e68a]/12 rounded p-2">Domain — WhoisJSON (RDAP is keyless)</div>
          <div className="border border-[#00e68a]/12 rounded p-2">Registry — Companies House, EDGAR (public)</div>
          <div className="border border-[#00e68a]/12 rounded p-2">Infra — Redis (Upstash + local), Postgres</div>
        </div>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <div className="bg-muted/30 border border-[#00e68a]/12 p-4 rounded-lg my-4 flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded">Org surface</span>
          <span className="text-xs font-mono text-muted-foreground">→</span>
          <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded">Related people</span>
          <span className="text-xs font-mono text-muted-foreground">→</span>
          <span className="inline-flex items-center gap-1 text-xs font-mono text-[#34f5a0] bg-[#00e68a]/10 px-2 py-1 rounded">
            <Mail className="h-3 w-3" /> Personal channel
          </span>
          <span className="text-xs font-mono text-muted-foreground">→</span>
          <span className="inline-flex items-center gap-1 text-xs font-mono text-[#00e68a] bg-[#00e68a]/10 px-2 py-1 rounded">
    ...
    content: (
      <>
        ...
        <div className="bg-muted/30 border border-[#00e68a]/12 p-4 rounded-lg my-4 flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs font-mono text-muted-foreground">→</span>
          <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded">api-server</span>
          <span className="text-xs font-mono text-muted-foreground">→</span>
        ...
        <ul className="space-y-3 text-sm text-muted-foreground mb-4">
          ...
          <li className="flex gap-3">
            <Shield className="w-5 h-5 text-[#34f5a0] shrink-0" />
            <div>
          ...
          <li className="flex gap-3">
            <AlertTriangle className="w-5 h-5 text-[#00e68a] shrink-0" />
            <div>
    ...
    content: (
      <>
        <ul className="space-y-3 text-sm text-muted-foreground">
          ...
          <li className="flex gap-3">
            <Crosshair className="w-5 h-5 text-[#34f5a0] shrink-0" />
            <div>
  ...
  return (
    <div className="atlas-page max-w-3xl py-8 sm:py-10">
      ...
      <div className="mb-6 flex flex-wrap gap-2" data-testid="manual-quick-links">
        <Link href="/reactor" className="inline-flex min-h-[36px] items-center gap-1.5 rounded-lg border border-emerald-400/25 bg-emerald-400/10 px-3 py-1.5 text-[11px] font-semibold text-emerald-100 hover:border-yellow-300/40">
          <Cpu className="h-3.5 w-3.5" /> Live reactor
        </Link>
        <Link href="/search" className="inline-flex min-h-[36px] items-center gap-1.5 rounded-lg border border-[#00e68a]/12 bg-card/50 px-3 py-1.5 text-[11px] font-semibold text-foreground hover:border-primary/40">
          Discover
        </Link>
        <Link href="/profiles" className="inline-flex min-h-[36px] items-center gap-1.5 rounded-lg border border-[#00e68a]/12 bg-card/50 px-3 py-1.5 text-[11px] font-semibold text-foreground hover:border-primary/40">
          Entity ledger
        </Link>
      </div>
      ...
      <input
        ...
        className="mb-5 w-full rounded-xl border border-[#00e68a]/10 bg-background/80 px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50 outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/30 focus-visible:border-emerald-400/40"
        autoComplete="off"
      />

      <div className="space-y-2" role="list">
        {visible.length === 0 && (
          <div className="rounded-lg border border-[#00e68a]/12 bg-card px-4 py-8 text-center text-sm text-muted-foreground" data-testid="manual-search-empty">
        ...
        {visible.map((section) => {
          ...
          return (
            <div key={section.id} className="border border-[#00e68a]/12 rounded-lg bg-card overflow-hidden" role="listitem">
              <button
                type="button"
              ...
              {open && (
                <div
                  ...
                  className="px-4 pb-5 border-t border-[#00e68a]/12 pt-4"
                >
                  {section.content}
                </div>
              )}
