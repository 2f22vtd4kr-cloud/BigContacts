...
function StepCard({ icon: Icon, name, description, status, metric, detail, durationMs }: StepCardProps) {
  return (
    <div className={cn(
      "border border-[#e85d1a]/12 rounded-2xl bg-card/30 p-3 sm:p-4 transition-all duration-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]",
      status === "idle"    && "border-[#e85d1a]/12 bg-card/20 opacity-50",
      status === "running" && "border-primary/50 bg-primary/5 shadow-[0_0_15px_rgba(232,93,26,0.1)]",
      status === "done"    && "border-[#e85d1a]/30 bg-[#e85d1a]/5",
    )}>
      <div className="flex items-start justify-between mb-2 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className={cn(
            ...
            status === "done"    && "bg-[#e85d1a]/20",
          )}>
            {status === "running" ? (
              <Loader2 className="w-3.5 h-3.5 text-primary animate-spin" />
            ) : status === "done" ? (
              <CheckCircle2 className="w-3.5 h-3.5 text-[#e85d1a]" />
            ) : (
...
function ResultCard({ result }: { result: SearchResult }) {
  ...
  const confColor =
    result.confidence === "high"   ? "text-[#f97316] border-[#e85d1a]/30 bg-[#e85d1a]/10" :
    result.confidence === "medium" ? "text-[#e85d1a] border-[#e85d1a]/30 bg-[#e85d1a]/10" :
                                     "text-muted-foreground border-[#e85d1a]/12 bg-card/30";
  ...
  return (
    <div className={cn(
      ...
      result.isHot ? "border-[#e85d1a]/30 bg-[#e85d1a]/5" : "border-[#e85d1a]/12 bg-card/30",
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0 mr-3">
          <div className="flex items-center gap-2 mb-1 min-w-0">
            ...
            {result.isHot && <Zap className="w-3 h-3 text-[#e85d1a] shrink-0" />}
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          <div className={cn(
            ...
            score >= 75 ? "text-[#f97316] bg-[#e85d1a]/10 border border-[#e85d1a]/30" :
            score >= 55 ? "text-primary bg-primary/10 border border-primary/20" :
                          "text-muted-foreground bg-muted border border-[#e85d1a]/12",
      ...
      <div className="space-y-1.5 mb-3">
        <ScoreBar label="Keywords" value={result.scores.bm25}      color="bg-[#e85d1a]" title="Keyword match (BM25)" />
        <ScoreBar label="Meaning"  value={result.scores.semantic}   color="bg-[#c2410c]" title="Semantic similarity" />
        <div className={cn("space-y-1.5", !expandedScores && "hidden sm:block")}>
          <ScoreBar label="Links"    value={result.scores.graph}      color="bg-[#e85d1a]" title="Graph / relationship strength" />
      ...
      {expanded && (
        <div className="mt-2 text-xs font-mono text-muted-foreground bg-background/50 border border-[#e85d1a]/12 rounded p-2 leading-relaxed">
          {result.reasoning}
        </div>
      )}
...
export default function DeepSearch() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 border-b border-[#e85d1a]/12 bg-card/50 px-4 sm:px-6 py-4">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative w-full">
            ...
            <input
              ...
              className="w-full bg-background border border-[#e85d1a]/12 rounded-lg pl-10 pr-4 h-[48px] md:h-auto md:py-2.5 text-sm font-mono text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary focus-visible:ring-2 focus-visible:ring-primary/40 transition-colors"
            />
          </div>
          <button
            type="submit"
            ...
            className={cn(
              ...
              loading || !query.trim()
                ...
                : "bg-primary text-primary-foreground hover:bg-primary/90 shadow-[0_0_15px_rgba(232,93,26,0.3)]",
            )}
          >
        ...
        {/* Filter panel */}
        {filtersOpen && (
          <div className="mt-3 p-4 bg-background border border-[#e85d1a]/12 rounded-lg space-y-4">
            {/* Asset types */}
            <div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      filterAssetTypes.includes(t)
                        ...
                        : "border-[#e85d1a]/12 text-muted-foreground hover:border-secondary/50",
                    )}
                  >
                    {t}
                  </button>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      filterSources.includes(j.value)
                        ? "bg-[#e85d1a]/20 border-[#e85d1a] text-[#e85d1a]"
                        : "border-[#e85d1a]/12 text-muted-foreground hover:border-[#e85d1a]/50",
                    )}
                  >
                    {j.label}
                  </button>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="text-xs font-mono text-muted-foreground border border-[#e85d1a]/12 bg-card/25 rounded-xl px-3 py-1.5 hover:border-orange-400/40 hover:text-orange-100 transition-colors"
              >
                {ex}
              </button>
            ))}
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {/* Empty DB notice */}
        {result?.isEmpty && (
          <div className="m-6 flex items-center gap-3 border border-[#e85d1a]/30 bg-[#e85d1a]/5 rounded-lg p-4">
            <AlertCircle className="w-4 h-4 text-[#e85d1a] flex-shrink-0" />
            <span className="text-xs font-mono text-[#e85d1a]">
        ...
        {(loading || result) && (
          <div className="flex flex-col md:flex-row h-full overflow-hidden">
            ...
            {/* Left: pipeline steps */}
            <div className="w-full md:w-80 xl:w-96 flex-shrink-0 max-h-[270px] md:max-h-none md:border-r border-[#e85d1a]/12 p-4 sm:p-5 overflow-y-auto">
              ...
              {result && (
                <div className="pt-3 border-t border-[#e85d1a]/12">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-5">
              {loading && !result && (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="border border-[#e85d1a]/12 bg-card/30 rounded-lg p-4 animate-pulse">
                ...
                <>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-3 text-xs font-mono text-muted-foreground">
                      <span className="flex items-center gap-1" title="Keyword match"><span className="w-2 h-2 rounded-full bg-[#e85d1a] inline-block" />Keywords</span>
                      <span className="flex items-center gap-1" title="Semantic similarity"><span className="w-2 h-2 rounded-full bg-[#c2410c] inline-block" />Meaning</span>
                      <span className="hidden sm:flex items-center gap-1" title="Graph links"><span className="w-2 h-2 rounded-full bg-[#e85d1a] inline-block" />Links</span>
                    </div>
                  </div>
        ...
        {!loading && !result && !error && (
          <div className="flex flex-col items-center justify-center h-full text-center px-6 py-12">
            <div className="grid h-14 w-14 place-items-center rounded-2xl border border-[#e85d1a]/12 bg-card/40 mb-4">
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
              <a
                href="/data-sources"
                className="inline-flex min-h-[36px] items-center rounded-lg border border-[#e85d1a]/12 bg-card/50 px-3 py-1.5 text-[11px] font-semibold text-foreground hover:border-primary/40"
              >
                Load registries
              </a>
              <a
                href="/reactor"
                className="inline-flex min-h-[36px] items-center rounded-lg border border-orange-400/30 bg-orange-400/10 px-3 py-1.5 text-[11px] font-semibold text-orange-100 hover:border-yellow-300/50"
              >
                Open live reactor
              </a>
            </div>
