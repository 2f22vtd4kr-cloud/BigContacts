...
export default function GraphViewer() {
  ...
  function drawNodeLabel(node: any, ctx: CanvasRenderingContext2D, globalScale: number) {
    ...
    const color = node.isTarget                              ? "#00e68a"
      : _nt === "HNWI"                                      ? "#00e68a"
  ...
  return (
    ...
    >
      ...
      <div className="flex md:hidden flex-col border-b border-[#00e68a]/12 bg-card/90 backdrop-blur z-30 flex-shrink-0"><div className="flex items-center gap-2 px-3 py-2">
        {entityIdFromUrl && (
          <Link
            href={`/profile/${entityIdFromUrl}`}
            className="w-9 h-9 flex items-center justify-center rounded bg-muted border border-[#00e68a]/12 text-muted-foreground hover:text-foreground flex-shrink-0"
        ...
        <button
          ...
          className="flex-1 flex items-center justify-between px-3 py-2 rounded bg-background border border-[#00e68a]/12 text-left"
        >
        ...
        <button
          ...
          className={cn(
            ...
            filterOpen || minScore > 0 || assetTypeFilter
              ...
              : "bg-muted border-[#00e68a]/12 text-muted-foreground hover:text-foreground"
          )}
          title="Filter"
        >
      ...
      {/* ── Mobile filter sheet ── */}
      {filterOpen && (
        <div className="md:hidden absolute left-3 right-3 top-[57px] z-50 bg-card border border-[#00e68a]/12 rounded shadow-2xl p-4 space-y-4">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    assetTypeFilter === type
                      ...
                      : "border-[#00e68a]/12 text-muted-foreground hover:border-primary/50"
                  )}
                >
          ...
          <button
            ...
            className="w-full text-[10px] font-mono text-muted-foreground hover:text-foreground border border-[#00e68a]/12 rounded py-1.5 transition-colors"
          >
            Clear Filters
          </button>
        </div>
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 hidden md:flex items-center space-x-2 max-w-[90vw]">
        ...
        {entityIdFromUrl && (
          <Link
            href={`/profile/${entityIdFromUrl}`}
            className="flex items-center space-x-1.5 bg-card/90 backdrop-blur border border-[#00e68a]/12 px-3 py-2 rounded text-xs font-mono text-muted-foreground hover:text-foreground transition-colors"
            title="Back to profile"
          >
        ...
        <div className="relative">
          <button
            onClick={() => setSelectorOpen((o) => !o)}
            className="flex min-h-[40px] items-center space-x-2 bg-card/90 backdrop-blur border border-[#00e68a]/12 px-3 py-2 rounded text-sm font-mono text-foreground hover:border-primary/50 transition-colors min-w-0 w-full sm:min-w-[220px] sm:w-auto"
          >
        ...
        {/* Zoom controls */}
        <div className="flex space-x-1 bg-card/90 backdrop-blur border border-[#00e68a]/12 p-1 rounded">
          <button
        ...
        <div className="relative">
          ...
          {filterOpen && (
            <div className="absolute top-full mt-2 right-0 w-64 bg-card border border-[#00e68a]/12 rounded shadow-2xl z-50 p-4 space-y-4 hidden md:block">
              <div>
                <label className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-2 block">
                  Min Signal Score: {minScore}%
                </label>
              ...
              <button
                onClick={() => { setMinScore(0); setAssetTypeFilter(null); setFilterOpen(false); }}
                className="w-full text-[10px] font-mono text-muted-foreground hover:text-foreground border border-[#00e68a]/12 rounded py-1 transition-colors"
              >
                Clear Filters
              </button>
            </div>
      ...
      {selectorOpen && (
        <div className={cn(
          "bg-card border border-[#00e68a]/12 shadow-2xl z-50 flex flex-col",
          ...
          <div className="p-2 border-b border-[#00e68a]/12 flex items-center space-x-2">
      ...
      {/* ── Legend ── */}
      <div className={cn(
        "absolute bottom-4 left-4 z-10 flex-col space-y-1 bg-card/80 backdrop-blur border border-[#00e68a]/12 p-2 md:p-3 rounded text-[10px] md:text-xs font-mono",
        ...
        <div className="flex items-center"><div className="w-2.5 h-2.5 rounded-full bg-[#00e68a] mr-1.5 md:mr-2" /> Corp</div>
        <div className="flex items-center"><div className="w-2.5 h-2.5 rounded-full bg-purple-500 mr-1.5 md:mr-2" /> Trust</div>
        <div className="flex items-center"><div className="w-2.5 h-2.5 rounded-full bg-[#00e68a] mr-1.5 md:mr-2" /> Gatekeeper</div>
      ...
      {(graphData as any)?.truncated && (
        <div className="absolute top-16 md:top-20 left-1/2 -translate-x-1/2 z-20 max-w-[90vw] px-3 py-1.5 rounded border border-[#00e68a]/40 bg-[#00e68a]/10 text-[10px] md:text-xs font-mono text-[#a7f3d0]/90 text-center">
      ...
      {isError && !isMockMode() && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 px-6 text-center space-y-3">
          <div className="grid h-14 w-14 place-items-center rounded-2xl border border-[#00e68a]/12 bg-card/50">
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-1">
            <Link href="/profiles" className="inline-flex min-h-[36px] items-center rounded-lg border border-[#00e68a]/12 bg-card/60 px-3 py-1.5 text-[11px] font-semibold text-foreground hover:border-primary/40">Entity ledger</Link>
            <Link href="/status" className="inline-flex min-h-[36px] items-center rounded-lg border border-[#00e68a]/12 bg-card/60 px-3 py-1.5 text-[11px] font-semibold text-foreground hover:border-primary/40">System status</Link>
          </div>
        </div>
      )}
      ...
      {!isError && allEntities !== undefined && allEntities.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 px-6 text-center space-y-3">
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-1">
            ...
            <Link href="/search" className="inline-flex min-h-[36px] items-center rounded-lg border border-[#00e68a]/12 bg-card/60 px-3 py-1.5 text-[11px] font-semibold text-foreground hover:border-primary/40">Discover</Link>
          </div>
        </div>
      )}
      ...
      {!isLoading && allEntities !== undefined && allEntities.length > 0 && gData.nodes.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-0 px-6 text-center space-y-3">
          ...
          <Link href="/reactor" className="inline-flex min-h-[36px] items-center rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-1.5 text-[11px] font-semibold text-emerald-100">Open live reactor</Link>
        </div>
      )}
      ...
      {!isLoading && gData.nodes.length === 1 && gData.links.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
          <div className="bg-card/90 border border-[#00e68a]/12 rounded-lg px-6 py-5 flex flex-col items-center gap-3 max-w-xs text-center shadow-xl">
      ...
      {selectedNode && (
        <div className={cn(
          "absolute bg-card/95 backdrop-blur-md border-[#00e68a]/12 z-20 flex flex-col animate-in",
          // Mobile: bottom sheet
          ...
          <div className="p-4 border-b border-[#00e68a]/12 flex justify-between items-center bg-muted/20">
          ...
          <div className="p-4 md:p-6 flex-1 overflow-y-auto space-y-4">
            ...
            <div className="space-y-2 font-mono text-sm">
              {selectedNode.bayesianScore != null && (
                <div className="flex justify-between items-center p-3 border border-[#00e68a]/12 bg-muted/10 rounded">
              ...
              {selectedNode.isCentral && !selectedNode.isTarget && (
                <div className="flex justify-between items-center p-3 border border-[#00e68a]/30 bg-[#00e68a]/5 rounded">
                  <span className="text-[#00e68a]/70">Role</span>
                  <span className="text-[#00e68a] font-bold">CENTRAL NODE</span>
                </div>
              )}
            ...
            {selectedNode.nodeType === "Gatekeeper" && (() => {
              ...
              return (
                <div className="border-t border-[#00e68a]/12 pt-4">
                  <div className="text-xs text-muted-foreground mb-2 font-mono uppercase tracking-wider flex items-center">
                    <Shield className="w-3 h-3 mr-1.5 text-[#00e68a]" /> How to Approach
                  </div>
                  <div className="text-xs font-mono text-[#00e68a] bg-[#00e68a]/5 border border-[#00e68a]/20 p-3 rounded leading-relaxed">
                    {approachVector}
                  </div>
                </div>
              );
            ...
            {selectedNode.metadata && selectedNode.nodeType !== "Gatekeeper" && (
              <div className="border-t border-[#00e68a]/12 pt-4">
                ...
                <pre className="text-xs text-primary whitespace-pre-wrap bg-background p-3 rounded border border-[#00e68a]/12 overflow-x-auto">
            ...
            <div className="space-y-2 pt-2">
              ...
              <button
                ...
                className="w-full py-2 flex items-center justify-center gap-1.5 bg-muted/20 hover:bg-muted/40 text-muted-foreground hover:text-foreground border border-[#00e68a]/12 text-xs font-mono uppercase tracking-wider transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
              >
      ...
      {ctxMenu && (
        <div
          className="fixed z-50 bg-card border border-[#00e68a]/12 rounded shadow-2xl py-1 min-w-[180px]"
          style={{ left: ctxMenu.x, top: ctxMenu.y }}
          onMouseLeave={() => setCtxMenu(null)}
        >
          <div className="px-3 py-1.5 border-b border-[#00e68a]/10">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              ...
              <div className="px-3 py-2 bg-muted/20 border border-[#00e68a]/12 rounded text-sm font-mono text-foreground/70">
                {relSourceName}
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-background border border-[#00e68a]/12 rounded text-sm font-mono text-foreground placeholder:text-muted-foreground/40 focus:border-primary/50 focus:outline-none"
              />
              {relSearchResults.length > 0 && !relTargetId && (
                <div className="absolute left-0 right-0 top-full mt-1 bg-card border border-[#00e68a]/12 rounded shadow-xl z-50 max-h-48 overflow-y-auto">
            ...
            <div>
              ...
              <select value={relType} onChange={(e) => setRelType(e.target.value)}
                className="w-full px-3 py-2 bg-background border border-[#00e68a]/12 rounded text-sm font-mono text-foreground focus:border-primary/50 focus:outline-none">
            ...
            <div>
              ...
              <textarea value={relNotes} onChange={(e) => setRelNotes(e.target.value)} rows={2}
                ...
                className="w-full px-3 py-2 bg-background border border-[#00e68a]/12 rounded text-sm font-mono text-foreground placeholder:text-muted-foreground/40 focus:border-primary/50 focus:outline-none resize-none" />
            </div>
          ...
          <DialogFooter className="gap-2">
            <DialogClose asChild>
              <button className="px-4 py-2 rounded border border-[#00e68a]/12 text-muted-foreground font-mono text-xs uppercase tracking-wider hover:text-foreground transition-colors">Cancel</button>
            </DialogClose>
