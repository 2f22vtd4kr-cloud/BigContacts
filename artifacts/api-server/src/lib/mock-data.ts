...
export async function seedMockData(): Promise<void> {
  ...
  const entityData = [
    ...
    },
    {
      ...
      notes: "Yacht management company for Morozov's 'Meridian Star'. Captain: Petros Anastasiou (+30 693 XXX XXXX). Receives all provisioning / berthing coordination — warm introduction vector.",
    ...
    },
    {
      ...
      notes: "Professional Hunter (PH) at Kariuki Safaris. Conducts annual Big Five hunts with Edward Fitzwilliam-Holt (Laikipia concession, Jan–Feb each year). Warm, professional relationship with target. Commission + referral standard in safari industry.",
      sourceRegistries: JSON.stringify(["Kenya Wildlife Service (PH License)", "CITES records"]),
      metadata: JSON.stringify({ approachVector: "WhatsApp + upcoming season booking angle", confidence: "HIGH" }),
      isHot: false,
    },
    {
