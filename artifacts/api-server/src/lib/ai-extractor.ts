...
export function buildPerplexityPrompt(
  ...
  entityType: string,
  country: string | null,
  context: { tradingName?: string | null; city?: string | null; realismDirective?: string } = {},
): string {
  ...
  const disambig = locationCtx
    ...
    : "";
  const realismDirective = context.realismDirective
    ? `\n\n${context.realismDirective}`
    : "";
...
ACCESS REALISM CONTRACT (always applies):
- Wealth, fame, press visibility, social accounts, assets, registries, switchboards, generic inboxes, FBOs, marinas, clubs, property managers, brokers, and hypothetical assistants/staff are evidence only. They are not access.
- Only report a direct route when the named person's public email/phone is explicitly published or corroborated with attribution, or an intermediary's attributable public vector and relationship are corroborated.
- Never construct, guess, pattern-fill, or infer an email, phone number, assistant, staff member, or relationship. If no route is supported, return null contact fields and state "No viable public access route found".
...
For EVERY named person: report an individual direct email only when explicitly published and attributable; personal LinkedIn /in/ URL and Twitter/X handle are context unless they provide a real route.
...
Do not construct or infer direct emails. An email pattern is context only and must never become a contact value.
...
Their individual named email address only when explicitly published and attributable. NOT info@, press@, media@, contact@, or any shared company inbox. Never construct from a domain pattern.
...
{
  "ownershipSummary": "one sentence: strongest ownership finding; email pattern is context only and must not be used to infer contact",
  ...
  "ownerResolutions": [
    {
      ...
      "email": "direct personal email only when explicitly published and attributable, otherwise null"
    }
  ],
  "sources": ["URLs used"]
}
...
export async function researchWithTavily(
  ...
  queryParts.push(context.realismDirective ? "realistic public access route evidence" : "owner contact email phone");
...
export async function researchWithExa(
  ...
  if (context.city) queryParts.push(context.city);
  if (country) queryParts.push(country);
  queryParts.push(context.realismDirective ? "realistic public access route evidence" : "owner contact email");
  const query = queryParts.join(" ");
