...
function contactChannel(gk: PathStep | null, _gkClass: GkClass): string {
  return gk
    ? "the verified public contact vector documented for this person"
    : "a verified public contact vector";
}

function commissionLine(_gkClass: GkClass, _pathScore: number): string {
  return "\n\nNo referral fee, private channel, or informal introduction is assumed. Proceed only if the documented public contact vector and relationship evidence support a legitimate approach.";
}
...
function timingAdvice(gkClass: GkClass, notes: string): string {
  ...
  if (gkClass === "GEOMETRA") {
    return "TIMING: No seasonal restriction. Use only the verified public contact vector and source-supported relationship context.";
  }
  return "";
}
...
Our interest in ${targetFull} remains active. If there is a more convenient format or timing for this conversation, we are entirely flexible — use only the verified public contact vector documented for this person.
