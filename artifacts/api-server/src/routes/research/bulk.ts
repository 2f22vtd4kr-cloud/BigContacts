      ...
      for (const targetEntity of targets) {
        try {
          ...
          const hasKnownInvestorConn = targetRelationships.some(r => {
           ...
           const hasCorroboratedIntermediary = targetRelationships.some(r => {
             if (r.targetType !== "Entity") return false;
             const e = allEntities.find(e => e.id === r.targetId);
             return e?.type === "Gatekeeper" &&
               Boolean(e.email?.trim() || e.phone?.trim()) &&
               (e.contactConfidence ?? 0) >= 50;
           });
          ...
          await db.update(entitiesTable)
           ...
           if (realism.classification === "research_only" || realism.classification === "bounded") {
