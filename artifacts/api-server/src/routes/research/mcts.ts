...
import { assessTargetRealism, buildRealismDirective } from "../../lib/target-realism";
...
router.post("/research/run", async (req, res): Promise<void> => {
  ...
  const hasGatekeeperConn = targetRelationships.some((r) => {
    ...
    return connEntity?.type === "Gatekeeper" &&
      ((connEntity.email?.trim() ?? "") !== "" || (connEntity.phone?.trim() ?? "") !== "") &&
      (connEntity.contactConfidence ?? 0) >= 50;
  });
  ...
  const realism = assessTargetRealism({
    ...
    hasCorroboratedIntermediary: hasGatekeeperConn,
  });

  // A research-only result is intentionally persisted without spending budget
  // on hybrid retrieval, orchestration, MCTS, or pitch generation.
  if (realism.classification === "research_only") {
    const [session] = await db.insert(researchSessionsTable).values({
      targetEntityId: entityId,
      winningPath: null,
      mctsSteps: null,
      crmStatus: "Research Review",
      bayesianScoreAtRuntime: targetEntity.bayesianScore ?? 0,
      pathScore: 0,
      generatedPitch: null,
      notes: `No outreach recommendation generated. ${realism.reason}`,
      realismClass: realism.classification,
      realismReason: realism.reason,
      researchBudget: realism.researchBudget,
    }).returning();
    ...
    res.status(201).json({
      ...
      algorithmPipeline: [{
        algo: "Preflight — Target Realism",
        contribution: `${realism.classification}: ${realism.reason}`,
        status: "review_only",
      }],
    });
    return;
  ...
  let hybridCount = 0;
  try {
    const { results: hybridResults, meta } = await hybridSearch(targetEntity.name, undefined, realism.classification === "bounded" ? 8 : 15);
    hybridMeta = meta;
  ...
  const mctsResult = realism.classification === "bounded"
    ? { winningPath: [], mctsSteps: [], pathScore: 0, crmStatus: "Research Review" }
    : runMcts(graph, targetVertexId, bestBfsPath, depth);
  ...
  try {
    if (realism.classification === "bounded") {
      throw new Error("bounded target — critic and MCTS outreach path disabled");
    }
  ...
  try {
    if (realism.classification === "bounded") {
      throw new Error("bounded target — outreach disabled");
    }
  ...
  const [session] = await db
    ...
    .values({
      ...
      notes: realism.classification === "bounded"
        ? `Research Review only. No viable access route corroborated. ${realism.reason}`
        : undefined,
      realismClass: realism.classification,
      realismReason: realism.reason,
      researchBudget: realism.researchBudget,
