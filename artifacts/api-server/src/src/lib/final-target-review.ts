...
export function deriveTargetResearchDisposition(review: Pick<FinalTargetReviewResult, "approvedContactValues"> & { approvedRelatedValues?: string[]; cardSummary?: string | null }): { disposition: TargetResearchDisposition; nextAction: string } {
  if (review.approvedContactValues.length > 0 || (review.approvedRelatedValues?.length ?? 0) > 0) {
    return { disposition: "contact_route_found", nextAction: "Keep approved routes and related findings on the target card; continue only if a stronger direct contact appears." };
  }
...
function exactMatch(value: string, allowed: readonly string[]): boolean { const v = value.trim(); return allowed.some((candidate) => candidate === v || candidate.trim().toLowerCase() === v.toLowerCase()); }
function publicationConflictKey(vectorType: string, value: string): string { const vector = vectorType.trim().toLowerCase(); const normalized = vector === "phone" ? value.replace(/\D/g, "") : value.trim().toLowerCase(); return `${vector}|${normalized}`; }
...
function collectEligibleContactValues(input: FinalTargetReviewInput): string[] {
  ...
  const organizationTarget = input.targetType === "Corporation" || input.targetType === "Corp" || input.targetType === "Trust";
  return input.candidates
    .filter((candidate) => candidate.state !== "rejected" && candidate.conflictCount === 0)
    .filter((candidate) => candidate.vectorType === "email" || candidate.vectorType === "phone" || candidate.vectorType === "social")
    .filter((candidate) => { if (!candidate.scopes?.length) return true; if (organizationTarget) return candidate.scopes.includes("organization") || candidate.scopes.includes("target_person"); return candidate.scopes.includes("target_person") || candidate.scopes.includes("organization") || candidate.scopes.includes("person_candidate"); })
    .map((candidate) => candidate.value);
}

function collectEligibleRelatedValues(input: FinalTargetReviewInput): string[] {
  const conflictedContactKeys = new Set(input.candidates.filter((candidate) => candidate.conflictCount > 0).filter((candidate) => ["email", "phone", "social"].includes(candidate.vectorType)).map((candidate) => publicationConflictKey(candidate.vectorType, candidate.value)));
  const fromCandidates = input.candidates
    .filter((c) => c.state !== "rejected" && c.conflictCount === 0)
    .filter((c) => c.sourceUrls.length > 0 && c.exactClaimObserved)
    .filter((c) => ["address", "domain", "name", "role", "organization", "ownership"].includes(c.vectorType) || c.vectorType === "email" || c.vectorType === "phone")
    .map((c) => c.value);
...
function scrubUnsupportedContactClaims(summary: string | null, approvedContacts: readonly string[]): string | null {
  ...
  const unsupported = [...emails, ...urls, ...phoneLike].some((token) => { const normalized = token.trim().toLowerCase().replace(/[),.;]+$/, ""); return !approved.includes(normalized); });
...
export function adjudicateFinalTargetReview(input: FinalTargetReviewInput, raw: unknown, reviewerSource: string): FinalTargetReviewResult {
  ...
  const approvedContactValues = Array.isArray(payload.approvedContactValues) ? payload.approvedContactValues.filter((value): value is string => typeof value === "string").filter((value, index, values) => values.indexOf(value) === index).filter((value) => exactMatch(value, eligibleContacts)) : [];
  const approvedRelatedValues = Array.isArray(payload.approvedRelatedValues) ? payload.approvedRelatedValues.filter((value): value is string => typeof value === "string").filter((value, index, values) => values.indexOf(value) === index).filter((value) => exactMatch(value, eligibleRelated)).slice(0, 12) : [];
  const relatedDescriptions = Array.isArray(payload.relatedDescriptions) ? payload.relatedDescriptions.filter((value): value is string => typeof value === "string").slice(0, approvedRelatedValues.length) : [];
  const cardSummaryRaw = typeof payload.cardSummary === "string" && payload.cardSummary.trim().length >= 12 ? payload.cardSummary.trim().slice(0, 800) : null;
  const roleHeadline = typeof payload.roleHeadline === "string" && payload.roleHeadline.trim().length >= 3 ? payload.roleHeadline.trim().slice(0, 200) : null;
  const proposedAssetIdentifiers = input.proposedAssets.map((asset) => asset.identifier);
  const approvedAssetIdentifiers = Array.isArray(payload.approvedAssetIdentifiers) ? payload.approvedAssetIdentifiers.filter((value): value is string => typeof value === "string").filter((value, index, values) => values.indexOf(value) === index).filter((value) => exactMatch(value, proposedAssetIdentifiers)) : [];
  ...
  return { decision, approvedContactValues, approvedAssetIdentifiers, cardSummary, roleHeadline, approvedRelatedValues, relatedDescriptions, reasons: reasons.length > 0 ? reasons : hasCardMaterial ? ["Published from exact values explicitly selected by the reviewer."] : ["No values were explicitly selected for publication. Run another target-scoped OSINT review."], reviewerSource };
}
