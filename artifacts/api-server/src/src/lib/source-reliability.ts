...
const SOURCE_PROFILES: Array<{ match: RegExp; profile: Omit<SourceReliability, "source"> }> = [
  ...
  { match: /^(?:wikidata|wikipedia)$/i, profile: { reliability: 0.58, identity: 0.6, ownership: 0.35, contact: 0.18, freshness: 0.5, rationale: "Useful discovery and corroboration lead; not sufficient alone for attribution." } },
  { match: /^(?:ai|groq|gemini|perplexity|tavily|exa|mcts|hybrid\s+retrieval)$/i, profile: { reliability: 0.4, identity: 0.42, ownership: 0.28, contact: 0.22, freshness: 0.55, rationale: "Derived or discovery output; must remain bound to cited public sources." } },
];
...
export function getSourceReliability(source: string | null | undefined): SourceReliability {
  ...
  return { source: label, ...(matched?.profile ?? { reliability: 0.3, identity: 0.3, ownership: 0.2, contact: 0.15, freshness: 0.35, rationale: "Source family is not registered; manual review is required." }) };
}
