...
type AgentAction = { action: "web_search"; query: string; provider: "serper" | "tavily" | "exa"; locale?: string; market?: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "visit"; url: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "footprint_email"; email: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "footprint_username_maigret"; username: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "footprint_username_sherlock"; username: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "domain_lookup"; domain: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "registry_search"; query: string; registry: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "harvest_domain"; domain: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "browser_fetch"; url: string; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number } | { action: "done"; findings: AgenticFinding[]; thought?: string; hypothesis?: string; purpose?: string; expectedInformationGain?: number };
...
async function toolWebSearch(query: string, provider: "serper" | "tavily" | "exa", locale?: string, market?: string, signal?: AbortSignal): Promise<{ text: string; urls: string[]; provider: string; providerFallback: string[] }> {
  // Transport resilience only: the Investigator still selects the research move
  // and provider. A failed provider may fall through to another configured search
  // capability, without prescribing the research/query sequence.
  const order: Array<"serper" | "tavily" | "exa"> = [provider, ...(["serper", "tavily", "exa"] as const).filter((candidate) => candidate !== provider)];
  const providerFallback: string[] = [];
  for (const candidate of order) {
    const result = candidate === "serper" ? await webSearchSerper(query, locale, market, signal) : candidate === "tavily" ? await webSearchTavily(query, signal) : await webSearchExa(query, signal);
    if (result && result.urls.length > 0) return { ...result, provider: candidate, providerFallback };
    if (candidate !== provider) providerFallback.push(candidate);
  }
  return { text: `${provider} and configured search fallbacks returned no usable result.`, urls: [], provider, providerFallback };
}
