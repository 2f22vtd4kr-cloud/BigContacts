...
function releaseProviderSlot(): void { activeAgenticProviderDecisions = Math.max(0, activeAgenticProviderDecisions - 1); providerWaiters.shift()?.(); }
...
/** Observation-only extractor: facts and provenance only; never target/person attribution. */
...
function findingsFromContactFacts(page: string, sourceUrl: string): AgenticFinding[] {
  ...
  for (const line of extractContactFactsFromHtml(page).split("\n")) {
    const email = line.match(/^EMAIL:\s*(\S+)$/i)?.[1];
    if (email) { const value = sanitizePublicEmail(email); if (value && !isTrashContactValue("email", value)) out.push({ vectorType: "email", value, personName: null, role: null, scope: "organization", sourceUrls: [sourceUrl], note: "Observed public email on visited page" }); }
    const phone = line.match(/^PHONE:\s*(.+)$/i)?.[1];
    if (phone) { const value = sanitizePublicPhone(phone); if (value && !isTrashContactValue("phone", value)) out.push({ vectorType: "phone", value, personName: null, role: null, scope: "organization", sourceUrls: [sourceUrl], note: "Observed public phone on visited page" }); }
  }
  return out;
}
...
async function webSearchSerper(query: string): Promise<{ text: string; urls: string[] } | null> {
  ...
  if (!key) return null;
  try {
    const response = await fetch("https://google.serper.dev/search", { method: "POST", headers: { "X-API-KEY": key, "Content-Type": "application/json" }, body: JSON.stringify({ q: query, num: 10, gl: "us", hl: "en" }), signal: AbortSignal.timeout(15_000) });
...
async function webSearchTavily(query: string): Promise<{ text: string; urls: string[] } | null> {
  ...
  if (!key) return null;
  try {
    const response = await fetch("https://api.tavily.com/search", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ query, search_depth: "advanced", include_answer: true, max_results: 8, include_raw_content: false }), signal: AbortSignal.timeout(18_000) });
    if (!response.ok) return null;
...
async function webSearchExa(query: string): Promise<{ text: string; urls: string[] } | null> {
  ...
  if (!key) return null;
  try {
    const response = await fetch("https://api.exa.ai/search", { method: "POST", headers: { "x-api-key": key, "Content-Type": "application/json" }, body: JSON.stringify({ query, type: "auto", numResults: 8, contents: { text: { maxCharacters: 1600 } } }), signal: AbortSignal.timeout(18_000) });
    if (!response.ok) return null;
...
async function toolVisit(url: string): Promise<string> {
  try {
    const response = await fetch(url, { signal: AbortSignal.timeout(15_000), headers: { "User-Agent": "Apex-Atlas/1.0", Accept: "text/html,application/xhtml+xml,application/pdf;q=0.9,*/*;q=0.8" }, redirect: "follow" });
  ...
  } catch (error: any) { return `visit failed for ${url}: ${error?.message || "error"}`; }
}
...
async function callGroqJson(prompt: string, signal?: AbortSignal): Promise<{ model: string; raw: string } | null> {
  ...
  for (const key of keys) for (const model of GROQ_CHAT_MODELS) {
    ...
    } catch (error: any) { recordAgenticLlmAttempt({ provider: "groq", model, promptChars: prompt.length, status: error?.name === "TimeoutError" ? "timeout" : "error", success: false, latencyMs: Date.now() - started, retryIndex: attempt, reason: error?.message || "exception" }); }
  }
  return null;
}
async function callMistralJson(prompt: string, signal?: AbortSignal): Promise<{ model: string; raw: string } | null> {
  ...
  for (const model of models) {
    ...
    } catch (error: any) { recordAgenticLlmAttempt({ provider: "mistral", model, promptChars: prompt.length, status: error?.name === "TimeoutError" ? "timeout" : "error", success: false, latencyMs: Date.now() - started, retryIndex: attempt, reason: error?.message || "exception" }); }
  }
  return null;
}
  ...
  try {
    ...
    for (const [name, fn] of orderedProviders) {
      try {
        ...
        setAgenticLlmHealth(true, result.model, null); return result;
      } catch (error: any) { errors.push(name + ":" + (error?.message || "failed")); }
    }
...
async function runAgenticWebResearchUnbounded(input: {
  targetName: string; companyName?: string | null; objective?: string; investigatorLlm?: "groq" | "mistral"; maxIterations?: number; hardTimeoutMs?: number; shouldCancel?: () => boolean | Promise<boolean>; jobId?: string | null;
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    if (input.shouldCancel) { try { if (await input.shouldCancel()) return { status: "completed", model: modelUsed, iterations: i, searches, visits, findings, modelFindings: [], stopReason: "CANCELLED", trajectory: history, error: "cancelled by operator" }; } catch { /* continue */ } }
    ...
    if (!action) { history.push(`step${i + 1}: parse_failure`); lastObservation = "The previous model response was not valid action JSON. Choose one allowed action and return exactly one JSON object."; continue; }
    ...
    if (action.action === "web_search") {
      searches += 1; history.push(`step${i + 1}: web_search provider=${action.provider} query=${action.query}`);
      ...
      emit("web_search", { query: action.query, provider: result.provider, summary: `${result.urls.length} URLs returned` }); continue;
    }
    if (action.action === "visit") {
      ...
      visited.add(action.url); visits += 1; history.push(`step${i + 1}: visit ${action.url}`);
      const page = await toolVisit(action.url); lastObservation = page;
      const observed = findingsFromContactFacts(page, action.url), filingContext = findingsFromProxyPage(page, action.url);
      ...
      emit("visit", { url: action.url, provider: "page-fetch", summary: observed.length ? `${observed.length} unattributed contact fact(s)` : "page read" }); continue;
    }
