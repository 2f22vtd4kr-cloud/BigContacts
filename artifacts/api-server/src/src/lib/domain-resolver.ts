...
export interface DomainCandidate { domain: string; score: number; source: string; }
export interface DomainResolution {
  domain: string | null;
  confidence: number;
  source: string;
  mxVerified: boolean;
  spfProvider: string | null;
...
export function isBlockedDomain(domain: string): boolean {
  if (HOSTING_DOMAINS.has(domain) || REGISTRY_DOMAINS.has(domain) || FREEMAIL_DOMAINS.has(domain)) return true;
  for (const blocked of [...HOSTING_DOMAINS, ...REGISTRY_DOMAINS]) if (domain.endsWith(`.${blocked}`)) return true;
  return false;
}
...
function officialContactPaths(domain: string): string[] {
  ...
  return ["contact", "contact-us", "team", "about", "leadership", "people", "executives", "management", "investor-relations", "press"].map(path => `${base}/${path}`);
}

type JsonMap = Record<string, unknown>;
function parseJson<T>(val: string | null | undefined, fallback: T): T { try { return val ? JSON.parse(val) as T : fallback; } catch { return fallback; } }
function extractUrlsFromText(text: string): string[] { return (text.match(/https?:\/\/(?:www\.)?([a-zA-Z0-9\-]+(?:\.[a-zA-Z0-9\-]+)+)[^\s"')>]*/g) ?? []).slice(0, 6); }

export async function resolveEmployerDomain(entity: { name: string; type: string; metadata: string | null; notes: string | null; sourceRegistries: string | null; }): Promise<DomainResolution> {
  ...
  for (const field of ["website", "officialWebsite", "companyWebsite", "entityWebsite", "homepage"]) {
    ...
    if (typeof val === "string" && val.length > 4) {
      ...
      if (domain && !isBlockedDomain(domain)) candidates.push({ domain, score: 0.90, source: "metadata-website" });
    }
  }
  for (const field of ["registeredOfficeAddress", "chWebsite"]) {
    const val = meta[field];
    if (typeof val === "string") for (const url of extractUrlsFromText(val)) {
      const domain = extractDomain(url);
      if (domain && !isBlockedDomain(domain)) candidates.push({ domain, score: 0.85, source: "companies-house-address" });
    }
  }
  const edgarUrl = typeof meta["edgarUrl"] === "string" ? meta["edgarUrl"] : null;
  if (edgarUrl) { /* EDGAR URLs reference sec.gov; the filer's own domain may be in notes. */ }
  if (entity.notes) for (const url of extractUrlsFromText(entity.notes)) {
    const domain = extractDomain(url);
    if (domain && !isBlockedDomain(domain)) candidates.push({ domain, score: 0.65, source: "notes-url" });
  }
  if (["Corporation", "Trust"].includes(entity.type) && entity.name.length >= 3) {
    try {
      const resp = await fetch(`https://api.gleif.org/api/v1/fuzzycompletions?field=entity.legalName&page%5Bsize%5D=1&q=${encodeURIComponent(entity.name)}`, { signal: AbortSignal.timeout(6_000), headers: { Accept: "application/json" } });
      if (resp.ok) {
        const data = await resp.json() as { data?: Array<{ attributes?: { value?: string } }> };
        const matched = data.data?.[0]?.attributes?.value;
        if (matched) {
          const slug = entity.name.toLowerCase().replace(/\s+(inc|llc|ltd|corp|plc|sa|ag|gmbh|nv|bv|srl|pty|lp|lp\.|co)\.?$/i, "").replace(/[^a-z0-9]/g, "").slice(0, 24);
          if (slug.length >= 3) for (const tld of [".com", ".net", ".org"]) candidates.push({ domain: `${slug}${tld}`, score: 0.28, source: "gleif-name-derived" });
        }
      }
    } catch { /* GLEIF unreachable — skip */ }
  }
  const deduped = new Map<string, DomainCandidate>();
  for (const c of candidates) { const existing = deduped.get(c.domain); if (!existing || c.score > existing.score) deduped.set(c.domain, c); }
  const sorted = [...deduped.values()].sort((a, b) => b.score - a.score).slice(0, 5);
  if (!sorted.length) return { domain: null, confidence: 0, source: "no-candidates", mxVerified: false, spfProvider: null, officialContactPaths: [], isOrganizationDomain: false };

  let chosen: DomainCandidate | null = null;
  let mxVerified = false;
  let spfProvider: string | null = null;
  // Every candidate that can become an outbound contact surface must pass MX verification.
  // In particular, GLEIF name-derived domains are guesses, not evidence of a registered website.
  ...
  if (!chosen) {
    return {
      domain: null,
      confidence: 0,
      source: "no-mx-verified-candidate",
      mxVerified: false,
      spfProvider: null,
      officialContactPaths: [],
      isOrganizationDomain: ["Corporation", "Trust"].includes(entity.type),
    };
  }

  const finalScore = Math.min(chosen.score + 0.10, 1.0);
  ...
  return { domain: chosen.domain, confidence: finalScore, source: chosen.source, mxVerified, spfProvider, officialContactPaths: officialContactPaths(chosen.domain), isOrganizationDomain: isOrg };
}
