...
function openingContext(target: { name: string; type: string }, companyName: string | null, caseId: number, objective: string, prior: string): string { return compactInvestigationContext({ raw: ["# Apex Atlas — Investigation Context", `Case: ${caseId}`, `Target: ${target.name}`, `Target type: ${target.type}`, `Company: ${companyName ?? "not established"}`, "## Bureau operating law", "Gemini is Boss. Gemini Right-hand is Right Hand Advisor. Groq/Mistral are Investigator capabilities. The Investigator owns the research trajectory. Deterministic code validates safety, provenance, budgets, lifecycle and promotion integrity; it does not prescribe research hops.", "## Objective", objective, "## Prior durable context", prior || "No prior target-scoped investigation context exists."].join("\n\n") }); }
export async function runCanonicalSingleTargetInvestigation(atlasJobId: string, targetId: number, options: CanonicalSingleTargetOptions = {}): Promise<void> {
  ...
  const caseRow = await ensureTargetCase(target, companyName, atlasJobId, options.existingCaseId); const depth = resolveResearchDepth({ explicit: options.researchDepth }); const hardTimeoutMs = Math.min(600_000, Math.max(30_000, Number.isFinite(options.targetTimeoutMs) ? Math.trunc(options.targetTimeoutMs!) : depth.agenticHardTimeoutMs)); const deadline = Date.now() + hardTimeoutMs; let caseState = parseCaseFile(caseRow.caseFile); let contextDocument = typeof caseState.contextDocument === "string" ? caseState.contextDocument : openingContext(target, companyName, caseRow.id, caseRow.objective, ""); let investigatorLlm: "groq" | "mistral" | null = typeof caseState.investigatorLlm === "string" && (caseState.investigatorLlm === "groq" || caseState.investigatorLlm === "mistral") ? caseState.investigatorLlm : null; let latestResult: Awaited<ReturnType<typeof runTargetContactAgent>> | null = null; let lastOversight: StoredOversight | null = null; let completedActs = 0; let deadlineExceeded = false; let cancelled = false;
  await db.update(researchCasesTable).set({ status: "active", currentAction: "gemini-opening-assignment", updatedAt: new Date() }).where(eq(researchCasesTable.id, caseRow.id)); await updateJob(atlasJobId, { status: "running", progress: 0, total: 1, atlasPhase: 0, atlasPhaseTotal: 1, message: `Gemini Boss opening assignment for ${target.name}…` });
  if (!investigatorLlm) {
    if (Date.now() >= deadline) deadlineExceeded = true; else {
      ...
      investigatorLlm = opening.investigatorLlm; if (!investigatorLlm) { await db.update(researchCasesTable).set({ status: "review", currentAction: "gemini-opening-assignment-failed", updatedAt: new Date() }).where(eq(researchCasesTable.id, caseRow.id)); await updateJob(atlasJobId, { status: "failed", progress: 1, message: `Gemini Boss did not select a usable Investigator for ${target.name}; no fallback permitted.`, result: JSON.stringify({ caseId: caseRow.id, opening }) }); return; }
      contextDocument = compactInvestigationContext({ raw: `${contextDocument}\n\n## Gemini Boss opening state\nmodel=${opening.model}\nselectedInvestigator=${investigatorLlm}\nreport=${opening.report}\nnextDirections=${opening.nextDirections.join(" | ")}\nuncertainties=${opening.uncertainties.join(" | ")}` }); caseState.contextDocument = contextDocument; caseState.investigatorLlm = investigatorLlm; await db.update(researchCasesTable).set({ caseFile: JSON.stringify(caseState), directorMode: "gemini_boss_active", directorModel: opening.model, currentAction: "investigator-act-1", updatedAt: new Date() }).where(eq(researchCasesTable.id, caseRow.id));
    }
  }
  for (let actNumber = 1; !deadlineExceeded; actNumber++) {
    ...
    const completedAct = latestResult.trajectoryRecords.at(-1);
    if (!completedAct) {
      lastOversight = null;
      break;
    }
    const oversight = await reviewTargetInvestigationAct({
      caseId: caseRow.id,
      controlTurn: caseRow.iteration + actNumber,
      runId: latestResult.executionId ?? `case-${caseRow.id}-run-${atlasJobId}`,
      targetName: target.name,
      targetType: target.type,
      objective: caseRow.objective,
      sharedContext: actContext,
      act: completedAct,
      recentActs: [],
    });
    lastOversight = oversight;
    ...
    const persistedOversight = readOversight(caseState, latestResult.executionId ?? null, actNumber);
    if (persistedOversight) lastOversight = persistedOversight;
    contextDocument = appendDurableActContext(typeof caseState.contextDocument === "string" ? caseState.contextDocument : actContext, actNumber, latestResult, lastOversight); caseState.contextDocument = contextDocument; await db.update(researchCasesTable).set({ caseFile: JSON.stringify({ ...caseState, contextDocument, lastOversight }), currentAction: latestResult.status === "completed" ? `investigator-act-${actNumber + 1}` : "investigator-act-failed", updatedAt: new Date() }).where(eq(researchCasesTable.id, caseRow.id)); if (latestResult.status === "cancelled" || (await getJob(atlasJobId))?.status === "cancelled") { cancelled = true; break; } if (latestResult.status !== "completed") break; if (!lastOversight || lastOversight.status !== "completed") break; if (lastOversight.action === "stop") break;
  }
