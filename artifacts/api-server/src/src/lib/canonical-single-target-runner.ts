...
async function ensureTargetCase(target: { id: number; name: string; type: string }, companyName: string | null, atlasJobId: string): Promise<TargetCase> {
  ...
  if (existing?.targetEntityId) return { ...existing, targetEntityId: existing.targetEntityId, iteration: Number(existing.iteration ?? 0), objective: existing.objective ?? `Investigate ${target.name} for realistic public contact routes.` };
...
export async function runCanonicalSingleTargetInvestigation(atlasJobId: string, targetId: number, options: CanonicalSingleTargetOptions = {}): Promise<void> {
  ...
  const incomplete = !latestResult || latestResult.status !== "completed" || resourceLimited || !stopped;
  await db.update(researchCasesTable).set({ status: incomplete ? "review" : "complete", currentAction: incomplete ? "investigator-incomplete-or-resource-limited" : "awaiting-human-review", lastDecisionAt: new Date(), updatedAt: new Date(), caseFile: JSON.stringify({ ...caseState, contextDocument, lastOversight, completedActs: acts, resourceLimited }) }).where(eq(researchCasesTable.id, caseRow.id));
  await updateJob(atlasJobId, { status: incomplete ? "failed" : "done", progress: Math.min(maxActs + 2, acts + 1), total: maxActs + 2, atlasPhase: Math.min(maxActs + 2, acts + 1), atlasPhaseTotal: maxActs + 2, outcome: incomplete ? "incomplete" : "complete", message: stopped ? `Gemini Boss explicitly stopped ${target.name} after ${acts} Investigator act(s).` : `Target investigation for ${target.name} preserved for review after ${acts} controlled act(s).`, result: JSON.stringify({ caseId: caseRow.id, investigator: latestResult ? { status: latestResult.status, model: latestResult.model, findings: latestResult.findings, searches: latestResult.searches, visits: latestResult.visits, contactOutcome: latestResult.contactOutcome, evidenceGraphs: latestResult.evidenceGraphs } : null, lastOversight, completedActs: acts, resourceLimited, hardTimeoutMs }), finishedAt: new Date().toISOString() });
  await clearActiveJobIfOwned("atlas-run", atlasJobId);
}
