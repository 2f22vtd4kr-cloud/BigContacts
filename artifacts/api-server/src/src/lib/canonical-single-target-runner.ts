...
type Oversight = { status: "completed" | "unavailable"; model: string; decision: string | null; reason: string | null; focusLanes: string[]; confidence: number | null; error: string | null };
type InvestigatorTrajectoryRecord = { turn: number; model: string; action: string; args: Record<string, unknown>; thought?: string; execution: string; observation?: string; observedUrls: string[]; findings: unknown[]; providerFallback?: string[]; stopReason?: string };
type BossState = { status: string; model: string; investigatorLlm: "groq" | "mistral" | null; decision: string | null; nextDirections: string[]; uncertainties: string[]; error: string | null };

function buildInvestigationContext(input: { caseId: number; targetName: string; targetType: string; companyName: string | null; iteration: number; rightHand: Oversight; boss: BossState; investigator?: { status: string; model: string; findings: number; searches: number; visits: number; stopReason?: string }; trajectory?: string[]; trajectoryRecords?: InvestigatorTrajectoryRecord[]; findingSummary?: string[]; priorContext?: string; phase: string }): string {
...
async function persistContext(caseId: number, contextDocument: string, iteration: number, actorRole: string, summary: string) {
  ...
  let caseFile: Record<string, unknown> = {}; try { caseFile = current?.caseFile ? JSON.parse(current.caseFile) as Record<string, unknown> : {}; } catch { caseFile = {}; }
...
async function ensureTargetCase(target: { id: number; name: string; type: string }, companyName: string | null, atlasJobId: string) {
  ...
  if (!created) throw new Error(`Unable to create investigation case for ${target.name}.`); return created;
}
...
async function reviewWithRightHand(targetName: string, contextDocument: string, opening: boolean, rightHand: Oversight): Promise<Oversight> {
  const prompt = opening ? `Review the exact target ${targetName} before Gemini assigns its Investigator.\n\nSHARED INVESTIGATION CONTEXT:\n${contextDocument}\n\nGive concise research priorities only. Do not browse and do not choose a contact. Return JSON with decision, reason, focusLanes, confidence.` : `Review the completed investigation for ${targetName}.\n\nSHARED INVESTIGATION CONTEXT:\n${contextDocument}\n\nAssess what changed, what remains uncertain, what evidence should be checked next, and whether any apparent contact claim is unsupported. Do not browse. Return JSON with decision, reason, focusLanes, confidence.`;
  try {
...
export async function runCanonicalSingleTargetInvestigation(atlasJobId: string, targetId: number, options: CanonicalSingleTargetOptions = {}): Promise<void> {
  ...
  const caseRow = await ensureTargetCase(target, companyName, atlasJobId); const caseId = caseRow.id; const baseIteration = Math.max(0, Number(caseRow.iteration ?? 0));
  ...
  const depth = resolveResearchDepth({ explicit: options.researchDepth }); const hardTimeoutMs = Math.max(30_000, options.targetTimeoutMs ?? depth.agenticHardTimeoutMs); const maxPasses = Math.max(1, Math.min(6, Math.ceil(depth.agenticMaxIterations / 8)));
  let rightHand: Oversight = { status: "unavailable", model: "none", decision: null, reason: null, focusLanes: [], confidence: null, error: null }; let boss: BossState = { status: "unavailable", model: "none", investigatorLlm: null, decision: null, nextDirections: [], uncertainties: [], error: null };
  ...
  let result: Awaited<ReturnType<typeof runTargetContactAgent>> | null = null; let trajectoryRecords: InvestigatorTrajectoryRecord[] = []; let trajectorySummary: string[] = []; let lastControl: TargetControlDecision | null = null; let pass = 0;
  for (pass = 1; pass <= maxPasses; pass++) {
    const direction = boss.nextDirections[0]; if (direction) contextDocument = `${contextDocument}\n\n## Gemini continuation direction\n${direction}`.slice(-32000);
    ...
    if (lastControl.action === "stop") break; if (pass === maxPasses) break; priorContext = contextDocument;
  }
  ...
  const resourceLimited = !stoppedByBoss; const incomplete = result.status !== "completed" || resourceLimited;
  ...
  await clearActiveJobIfOwned("atlas-run", atlasJobId); const current = await getJob(atlasJobId); if (!current) return;
}
