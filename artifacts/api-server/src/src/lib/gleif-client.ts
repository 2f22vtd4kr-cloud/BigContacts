...
export async function searchGleif(
  ...
  if (!resp.ok) {
    ...
    throw new Error(`GLEIF ${resp.status}: ${body.slice(0, 200) || resp.statusText}`);
  }
  ...
  return records.map((r: any): GleifResult => {
    ...
    const legalName: string = entity?.legalName?.name ?? entity?.otherNames?.[0]?.name ?? "Unknown";
    const jurisdiction: string = entity?.jurisdiction ?? entity?.legalAddress?.country ?? "Unknown";
    const addr = entity?.legalAddress;
    const addrStr = addr
      ? [...(addr.addressLines ?? []), addr.city, addr.postalCode, addr.country].filter(Boolean).join(", ")
      : undefined;
