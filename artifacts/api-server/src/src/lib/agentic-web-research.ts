...
function renumberTrajectory(value: string, turn: number): string {
  return value.replace(/^step\d+:/, `step${turn}:`);
}

function enrichObjective(base: string, context: {
  sharedContext: string;
  direction: string | null;
  records: CoreResult["trajectoryRecords"];
}): string {
  const recent = context.records.slice(-12).map((record) => ({
    turn: record.turn,
    action: record.action,
    execution: record.execution,
    args: record.args,
    observation: typeof record.observation === "string" ? record.observation.slice(0, 4000) : undefined,
    observedUrls: record.observedUrls.slice(0, 12),
    findings: record.findings.slice(0, 10),
  }));
...
/**
 * Canonical Investigator entrypoint.
 *
 * Target runs are deliberately stepped one action at a time so the completed
 * action crosses the durable observation boundary and is reviewed by the
 * DeepSeek Right Hand + Gemini Boss before another Investigator action is
 * permitted. Discovery retains the core multi-step path because it has no
 * target-scoped Boss/Right-hand case to supervise at this boundary.
 */
  ...
  return withAgenticExecutionScope(scope, async () => {
    ...
    try {
      for (let actionTurn = 1; actionTurn <= (input.maxIterations ?? 40); actionTurn++) {
        if (overallController.signal.aborted || input.signal?.aborted) {
          return { status: "cancelled", model, iterations: actionTurn - 1, searches, visits, findings, modelFindings, stopReason: "CANCELLED", trajectory, trajectoryRecords: records.slice(-100), error: "cancelled by operator" };
        }

        const remaining = deadline - Date.now();
        if (remaining <= 0) {
          return { status: "timeout", model, iterations: actionTurn - 1, searches, visits, findings, modelFindings, stopReason: "HARD_TIMEOUT", trajectory, trajectoryRecords: records.slice(-100), error: `hard timeout ${requestedHardTimeout}ms` };
        }

        const perActTimeout = Math.max(30_000, Math.min(55_000, remaining));
        const actInput: RunInput = {
          ...input,
          objective: enrichObjective(objective, { sharedContext: oversightContext.contextDocument, direction, records }),
          maxIterations: 1,
        ...
        if (actRecord) {
          ...
          oversight = await reviewTargetInvestigationAct({
            ...
            controlTurn: actionTurn,
            targetName: input.targetName,
            targetType: oversightContext.targetType,
            objective,
            sharedContext: oversightContext.contextDocument,
            act: normalizedRecord,
            recentActs: records.slice(-12),
          });
          direction = oversight.direction ?? direction;

          if (actRecord.action === "done" || oversight.action === "stop" || oversight.status !== "completed") {
            return {
              status: actResult.status === "completed" ? "completed" : actResult.status,
              model,
              iterations: actionTurn,
              searches,
              visits,
              findings,
              modelFindings,
        ...
        if (actResult.status !== "completed" || actResult.stopReason !== "ITERATION_BUDGET") {
          return { ...actResult, searches, visits, findings, modelFindings, trajectory, trajectoryRecords: records.slice(-100) };
        }
      }
