...
export async function runAgenticWebResearch(input: RunInput): Promise<CoreResult> {
  ...
  return withAgenticExecutionScope(scope, async () => {
    ...
    const overallController = new AbortController(); const abortExternal = () => overallController.abort(); input.signal?.addEventListener("abort", abortExternal, { once: true });
    const deadline = startedAt + requestedHardTimeout; const deadlineTimer = setTimeout(() => overallController.abort(), requestedHardTimeout);
    ...
    try {
      for (let actionTurn = 1; actionTurn <= (input.maxIterations ?? 40); actionTurn++) {
        ...
        if (actRecord) {
          ...
          records = [...records, normalizedRecord].slice(-100); trajectory = [...trajectory, ...actResult.trajectory.map((line) => renumberTrajectory(line, actionTurn))].slice(-100);
          if (actResult.modelFindings.length) modelFindings = [...modelFindings, ...actResult.modelFindings]; if (actRecord.findings.length) findings = [...findings, ...(actRecord.findings as CoreResult["findings"])];
