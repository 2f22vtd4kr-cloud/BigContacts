...
function buildStepPrompt(input: {
  ...
  const discoveryContract = /^Discovery slot\b/i.test(input.targetName)
    ? `\nDISCOVERY OUTPUT CONTRACT (model-owned, not a search script): Invent your own queries. Prefer concrete business/ownership/family-office/operator surfaces; do not walk Forbes/billionaire rankings. Search snippets are leads — visit the page before claiming identity. If you establish a real named person that YOU judge worth promoting, emit action=done with a finding containing personName="Full Name" (or value="person: <Full Name> | role | company"), scope="candidate", promotionDecision="promote", promotionReason="brief reason", and sourceUrls containing the exact HTTPS page you actually observed. For discovery, promotionDecision is mandatory on every finding; never omit it. A valid promotion looks like: {"action":"done","findings":[{"vectorType":"other","value":"person: <Full Name> | Founder | Example Co","personName":"<Full Name>","role":"Founder","scope":"candidate","sourceUrls":["https://example.com/about"],"note":"Named founder on the visited company page","promotionDecision":"promote","promotionReason":"Exact person identity and role are established on the visited source."}],"thought":"..."} If you do not judge the person worth promoting, emit promotionDecision="reject" or findings=[]. A personName-only finding is valid only when the required promotion fields are also present. Do not emit titles, sectors, companies, or prose fragments as people. If identity is not established, return done with findings=[].\n`
    : "";
