 ...
 * Identity and contact values may be observed on different pages; this is
 * intentional multi-source attribution, not concatenation of unobserved data.
 */
function claimAppearsInObservedMaterial(finding: AgenticFinding, records: AgenticTrajectoryRecord[]): boolean {
  ...
  const personTokens = finding.scope === "candidate" && finding.personName
    ...
    : [];
  let valueObserved = !exactValueRequired;
  let identityObserved = personTokens.length === 0;
  ...
  for (const record of records) {
    ...
    const matchedSources = record.observedUrls.filter((url) => sourceSet.has(url));
    if (!matchedSources.length) continue;
    const observation = record.observation.toLowerCase();
    const hasValue = !exactValueRequired || observation.includes(value);
    const hasIdentity = !personTokens.length || personTokens.every((token) => observation.includes(token));
...
async function resolveSelectedInvestigator(input: { investigatorLlm?: "groq" | "mistral"; caseId?: number; entityId: number; jobId?: string }): Promise<{ investigator: "groq" | "mistral"; caseId: number } | null> {
  ...
  try {
    const state = JSON.parse(row.caseFile) as Record<string, unknown>;
    if (state.atlasJobId !== input.jobId) return null;
    const selected = state.investigatorLlm;
    if (selected !== "groq" && selected !== "mistral") return null;
    if (input.investigatorLlm && input.investigatorLlm !== selected) return null;
    return { investigator: selected, caseId: input.caseId! };
  } catch { return null; }
}
export async function runTargetContactAgent(input: { entityId: number; caseId?: number; targetName: string; companyName?: string | null; jobId?: string; maxIterations?: number; hardTimeoutMs?: number; investigatorLlm?: "groq" | "mistral"; contextDocument?: string; shouldCancel?: () => boolean | Promise<boolean>; onInvestigationAct?: (step: InvestigationAct) => void | Promise<void> }): Promise<TargetContactAgentResult> {
  ...
  const selected = await resolveSelectedInvestigator(input);
  if (!selected) { logger.warn({ entityId: input.entityId, caseId: input.caseId, jobId: input.jobId }, "[target-agent] refusing Investigator run without an exact durable case/job/provider binding"); return { ...empty(), status: "unavailable" }; }
  ...
  const evidenceGraphs = buildEvidenceGraphs(backedFindings, agentic.trajectoryRecords, agentic.executionId ?? null);
  const multiSourceGraphs = evidenceGraphs.filter(graphHasIndependentCorroboration);
  ...
  const provenance: InvestigatorPromotionProvenance | undefined = agentic.executionId ? { caseId: selected.caseId, runId: agentic.executionId } : undefined;
  if (input.shouldCancel && await input.shouldCancel()) return { status: "cancelled", model: agentic.model, findings: 0, searches: agentic.searches, visits: agentic.visits, trajectory: agentic.trajectory.slice(-80), trajectoryRecords: agentic.trajectoryRecords.slice(-100), evidenceGraphs: [], phone: null, email: null, phoneSource: null, contactOutcome: null };
  if (input.jobId) { const currentJob = await getJobForTargetAgent(input.jobId); if (!currentJob) return { status: "cancelled", model: agentic.model, findings: 0, searches: agentic.searches, visits: agentic.visits, trajectory: agentic.trajectory.slice(-80), trajectoryRecords: agentic.trajectoryRecords.slice(-100), evidenceGraphs: [], phone: null, email: null, phoneSource: null, contactOutcome: null }; }
