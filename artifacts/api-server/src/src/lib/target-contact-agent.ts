...
export async function runTargetContactAgent(input: { entityId: number; targetName: string; companyName?: string | null; jobId?: string; maxIterations?: number; hardTimeoutMs?: number }): Promise<TargetContactAgentResult> {
  ...
  const objective = [
    ...
    "Act like a strong human public-web researcher with a bounded budget. The goal is an attributable, realistic route to this person, not fame, wealth ranking, or generic company contact information.",
    ...
    "Never invent a contact, relationship, person, or URL. Every finding needs an exact public source URL. Keep organization inboxes and switchboards in organization scope, never as personal contacts.",
  ...
  logger.info({ entityId: input.entityId, name, status: agentic.status, model: agentic.model, findings: agentic.findings.length, phone: ent?.phone ?? null, outcome }, "[TargetAgent] Card updated from free dig");
  ...
  return { status: mapped, model: agentic.model, findings: agentic.findings.length, searches: agentic.searches, visits: agentic.visits, phone: ent?.phone ?? null, email: ent?.email ?? null, phoneSource: ent?.phoneSource ?? null, contactOutcome: outcome };
}
