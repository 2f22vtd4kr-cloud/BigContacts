...
/** Require the claim and candidate identity to co-occur in one successful observation. */
function claimAppearsInObservedMaterial(finding: AgenticFinding, records: AgenticTrajectoryRecord[]): boolean { if (!records.length) return false; const sources = new Set(finding.sourceUrls.map((url) => { try { return new URL(url).href; } catch { return ""; } }).filter(Boolean)); const value = finding.value.trim().toLowerCase(); const exactValueRequired = ["email", "phone", "linkedin", "website", "social"].includes(finding.vectorType); const tokens = finding.scope === "candidate" && finding.personName ? finding.personName.toLowerCase().split(/[^a-z0-9]+/).filter((t) => t.length >= 2) : []; for (const record of records) { if (record.execution !== "success" || typeof record.observation !== "string") continue; if (!record.observedUrls.some((url) => sources.has(url))) continue; const text = record.observation.toLowerCase(); if (exactValueRequired && !text.includes(value)) continue; if (tokens.length && !tokens.every((t) => text.includes(t))) continue; return true; } return false; }
...
async function loadMountedCaseContext(caseId: string | number | undefined): Promise<string | null> { if (caseId == null) return null; const numericId = Number(caseId); if (!Number.isInteger(numericId) || numericId <= 0) throw new Error("Invalid investigation case ID; durable case context cannot be mounted."); const [row] = await db.select({ caseFile: researchCasesTable.caseFile }).from(researchCasesTable).where(eq(researchCasesTable.id, numericId)).limit(1); if (!row?.caseFile) throw new Error(`Investigation case ${numericId} has no durable case file.`); let parsed: Record<string, unknown>; try { parsed = JSON.parse(row.caseFile) as Record<string, unknown>; } catch { throw new Error(`Investigation case ${numericId} has an unreadable case file.`); } const document = typeof parsed.contextDocument === "string" ? parsed.contextDocument.trim() : ""; if (!document) throw new Error(`Investigation case ${numericId} has no durable context document.`); return document.slice(0, 28000); }

async function ensureDiscoveryCaseContext(input: { mode?: "target" | "discovery"; targetName: string; objective?: string; investigatorLlm?: "groq" | "mistral"; jobId?: string }): Promise<number | null> {
  if (input.mode !== "discovery" || input.jobId == null) return null;
  const [created] = await db.insert(researchCasesTable).values({ caseType: "discovery", status: "active", directorMode: "gemini_boss", directorProvider: "gemini", directorModel: "pending", objective: (input.objective ?? "Canonical Atlas model-owned discovery").slice(0, 10000), motivation: "Durable memory for canonical Atlas Investigator discovery.", openingPrompt: "Investigator chooses every research action. This case is memory/state, not a deterministic research plan.", caseFile: JSON.stringify({ caseType: "discovery", contextDocument: ["CANONICAL ATLAS DISCOVERY CASE", `JOB: ${input.jobId}`, `INVESTIGATOR: ${input.investigatorLlm ?? "unassigned"}`, `OBJECTIVE: ${(input.objective ?? "").slice(0, 8000)}`, "STATE: Discovery mode; no person target is implied.", "TRAJECTORY: []"].join("\n"), investigationTimeline: [], investigatorTrajectory: [], investigatorTrajectoryRecords: [], jobId: input.jobId }), currentAction: "canonical-investigator-discovery", iteration: 0 }).returning({ id: researchCasesTable.id });
...
export async function runBureauAgenticWebPass(input: { mode?: "target" | "discovery"; targetName: string; companyName?: string | null; objective?: string; investigatorLlm?: "groq" | "mistral"; caseId?: string | number; jobId?: string; maxIterations?: number; hardTimeoutMs?: number; entityId?: number; persist?: boolean; shouldCancel?: () => boolean | Promise<boolean>; onInvestigationAct?: (step: { action: string; provider?: string; query?: string; url?: string; summary?: string }) => void | Promise<void> }): Promise<BureauAgenticPassResult> {
  ...
  } catch (err: any) { logger.warn({ err: err?.message, target: name, mode }, "[Bureau] Agentic web pass failed"); return { status: "error", model: "none", iterations: 0, searches: 0, visits: 0, findings: [], contactEvidence: [], trajectory: [], error: err?.message ?? "agentic pass failed" }; }
}
