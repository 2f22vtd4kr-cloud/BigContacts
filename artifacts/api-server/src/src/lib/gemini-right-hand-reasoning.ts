...
async function request(system: string, user: string): Promise<GeminiRequestResult> {
  const apiKey = key(); if (!apiKey) return { raw: "", error: "GEMINI_RIGHT_HAND_API_KEY is not configured.", model: GEMINI_RIGHT_HAND_MODEL }; const chain = modelChain(); const failures: string[] = []; const configuredRequestTimeoutMs = requestTimeoutMs(); const configuredOverallTimeoutMs = overallTimeoutMs(); const deadline = Date.now() + configuredOverallTimeoutMs; const systemPrompt = `${apexOrientationCompact("right_hand")}\n\n${system}`; const requestPayload = (modelName: string, userPrompt: string) => ({ system_instruction: { parts: [{ text: systemPrompt }] }, contents: [{ role: "user", parts: [{ text: userPrompt }] }], generationConfig: { maxOutputTokens: 768, responseMimeType: "application/json", ...(isGemini3Model(modelName) ? { thinkingConfig: { thinkingLevel: "low" } } : {}) } });
