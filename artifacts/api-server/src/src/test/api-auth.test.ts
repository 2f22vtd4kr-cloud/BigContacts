...
describe("API authentication", () => {
  it("leaves health probes public", () => {
    expect(run("/api/healthz", "GET").nextCalled).toBe(true);
  });

  it("fails closed when the token is not configured", () => {
    const result = run("/api/entities", "GET");
    expect(result.nextCalled).toBe(false);
    expect(result.response.statusCode).toBe(503);
  });

  it("rejects missing and incorrect credentials", () => {
    process.env.APEX_API_AUTH_TOKEN = "x".repeat(32);
    expect(run("/api/entities", "GET").response.statusCode).toBe(401);
    expect(run("/api/entities", "GET", "Bearer wrong").response.statusCode).toBe(401);
  });
