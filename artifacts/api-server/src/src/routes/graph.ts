...
function vertexToNode(v: GraphVertex, centralSet: Set<string>, targetId: string) { return { id: v.id, label: v.label, nodeType: v.nodeType, bayesianScore: v.bayesianScore ?? null, estimatedValue: v.estimatedValue ?? null, metadata: v.metadata ?? null, nationality: v.nationality ?? null, contactConfidence: v.contactConfidence ?? null, contactEmail: v.contactEmail ?? null, contactPhone: v.contactPhone ?? null, contactOutcome: v.contactOutcome ?? null, isTarget: v.id === targetId, isCentral: centralSet.has(v.id) }; }
function arcToEdge(arc: GraphArc) { return { id: arc.id.replace("_rev", ""), source: arc.source, target: arc.target, label: arc.label, strength: arc.strength ?? null, provenanceScore: arc.provenanceScore ?? null, citationCount: arc.citationCount ?? 0, freshnessScore: arc.freshnessScore ?? null, evidenceStatus: arc.evidenceStatus ?? "review" }; }
...
router.get("/graph/entity/:id", async (req, res): Promise<void> => {
  ...
  for (const e of subEdges) { const baseId = e.id.replace("_rev", ""); if (!seenEdgeIds.has(baseId)) { seenEdgeIds.add(baseId); uniqueEdges.push(e); } }
  res.json({ nodes: subNodes.map((v) => vertexToNode(v, topCentral, centerVertexId)), edges: uniqueEdges.map(arcToEdge), centralNodeId: centerVertexId, depth, truncated: truncated || undefined });
});
...
router.get("/graph/path", async (req, res): Promise<void> => {
  ...
  if (sourceId === targetId) {
    res.json({ found: true, path: [vertexToNode({ id: `e:${sourceId}`, label: sourceEntity.name, nodeType: sourceEntity.type, bayesianScore: sourceEntity.bayesianScore }, new Set(), `e:${targetId}`)], edges: [], hops: 0, pathScore: 1, recommendation: "Source and target are the same entity." });
    return;
  }
  ...
  if (!pathResult) {
    res.json({ found: false, path: [], edges: [], hops: 0, pathScore: null, recommendation: "No connection path was found within the loaded graph neighborhood." });
    return;
  }
  const centralityRanking = computeCentrality(graph);
  const topCentral = new Set(centralityRanking.slice(0, 5).map((c) => c.vertexId));
  const pathNodes = pathResult.path.map((vId) => graph.vertices.get(vId)).filter((v): v is GraphVertex => v !== undefined).map((v) => vertexToNode(v, topCentral, targetVId));
  const pathEdges = pathResult.arcs.map(arcToEdge);
  const hops = pathResult.path.length - 1;
  const avgProv = pathEdges.length > 0 ? pathEdges.reduce((s, e) => s + (e.provenanceScore ?? 0.35), 0) / pathEdges.length : 1;
  ...
  const recommendation = gatekeeperNode
    ? `Path includes gatekeeper ${gatekeeperNode.label}. Path confidence: ${(pathScore * 100).toFixed(0)}%.`
    : `${hops}-hop connection path identified with ${(pathScore * 100).toFixed(0)}% path confidence.`;
  res.json({ found: true, path: pathNodes, edges: pathEdges, hops, pathScore: parseFloat(pathScore.toFixed(3)), recommendation });
});
