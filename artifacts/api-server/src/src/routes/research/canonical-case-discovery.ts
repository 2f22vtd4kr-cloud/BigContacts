...
/**
 * Canonical replacement for the legacy mixed-lane discovery handler. The old
 * route remains mounted for case CRUD, but this handler is mounted first so the
 * public run-discovery operation is always Gemini/DeepSeek coordination followed
 * by a selected Groq/Mistral free-ReAct Investigator.
 */
router.post("/research/bureau/cases/:caseId/run-discovery", async (req, res): Promise<void> => {
  ...
  const [current] = await db.select().from(researchCasesTable)
    .where(eq(researchCasesTable.id, caseId)).limit(1);
  if (!current) {
  ...
  await updateJob(jobId, {
    ...
    progress: 0,
    total: 4,
    message: "DeepSeek Right-hand reviewing the discovery mission…",
  });
  await db.update(researchCasesTable).set({ status: "active", currentAction: "canonical-investigator-discovery", updatedAt: new Date() })
  ...
  await db.insert(researchCaseEventsTable).values({
    ...
    summary: "Canonical Investigator discovery assigned; no fixed web or registry lane is used.",
    payload: JSON.stringify({ jobId, architecture: "gemini-boss + deepseek-right-hand + selected-groq-or-mistral-investigator" }),
  });
  ...
  void (async () => {
    try {
      ...
      const boss = await runGeminiBossDiscovery({
        objective: String(file.humanBrief?.objective ?? "").slice(0, 5000),
        motivation: String(file.humanBrief?.motivation ?? "").slice(0, 2000),
        geography: String(file.humanBrief?.geography ?? "").slice(0, 800),
        exclusions: Array.isArray(file.humanBrief?.exclusions) ? file.humanBrief.exclusions : [],
        rightHandAdvice: rightHand,
        startingLane: "canonical model-selected discovery",
      });
      ...
      const report = {
        ...
        summary: `Canonical Investigator discovery: ${discovery.findings.length} source-backed finding(s); ${admitted.length} explicit admission candidate(s).`,
        findings: discovery.trajectory.slice(-12),
        candidateNames: admitted,
        ...
        discoveredCandidates: [
          ...
          ...admitted.map((name) => {
            ...
            return {
              ...
              relevance: "Explicit Investigator discovery admission candidate",
              reachability: "Requires target-scoped Investigator research",
              sourceUrls: finding?.sourceUrls ?? [],
              contactEvidence: [],
              state: "review_only",
              admittedEntityId: null,
            };
          }),
        ],
        currentProgress: {
          ...(file.currentProgress ?? {}),
          reportCount: Number(file.currentProgress?.reportCount ?? 0) + 1,
          completedLanes: [...new Set([...(file.currentProgress?.completedLanes ?? []), "broad-web"])],
          openQuestions: [],
      ...
      for (const name of admitted) {
        ...
        if (!entityId) {
          const [created] = await db.insert(entitiesTable).values({
            ...
            notes: "Review-only discovery candidate; requires target-scoped Investigator research.",
            metadata: JSON.stringify({ reviewOnly: true, admission: "investigator-explicit", caseId, sourceUrls: finding?.sourceUrls ?? [] }),
          }).returning({ id: entitiesTable.id });
          entityId = created?.id ?? null;
          if (entityId) materialized += 1;
        }
        if (entityId && finding?.sourceUrls?.length) {
          await persistSourceBackedBureauContactsForEntity(entityId, [{
            ...
            role: finding.role,
            sourceUrls: finding.sourceUrls,
            note: "Explicit Investigator discovery admission; review-only.",
            tier: "candidate",
            state: "review_only",
            promote: false,
          }], "canonical-agentic-discovery", jobId);
        }
      }
      ...
      const message = error instanceof Error ? error.message : "Canonical case discovery failed.";
