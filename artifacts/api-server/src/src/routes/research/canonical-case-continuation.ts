...
router.post("/research/bureau/cases/:caseId/run-next-pass", async (req, res): Promise<void> => {
  ...
  const shouldCancel = async (): Promise<boolean> => { const job = await getJob(jobId); return !job || job.status === "cancelled" || job.status === "failed"; };
  try {
  ...
  void (async () => {
    ...
    } catch (error) { const message = error instanceof Error ? error.message : "Canonical case continuation failed."; const cancelled = await shouldCancel().catch(() => false); await db.update(researchCasesTable).set({ status: "review", currentAction: cancelled ? "canonical-continuation-cancelled" : "canonical-continuation-error", updatedAt: new Date() }).where(eq(researchCasesTable.id, caseId)); await updateJob(jobId, { status: cancelled ? "cancelled" : "failed", outcome: "incomplete", message, finishedAt: new Date().toISOString() }); }
