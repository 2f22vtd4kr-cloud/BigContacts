...
router.post("/research/bureau/cases/:caseId/run-next-pass", async (req, res): Promise<void> => {
  ...
  const shouldCancel = async (): Promise<boolean> => { const job = await getJob(jobId); return Boolean(!job || job.status === "cancelled" || job.status === "failed"); };
  try {
  ...
  void (async () => {
    try {
      if (await shouldCancel()) { throw new Error("Discovery continuation cancelled before Right-hand review."); }
      ...
      if (await shouldCancel()) { throw new Error("Discovery continuation cancelled after Right-hand review."); }
      ...
      if (await shouldCancel()) { throw new Error("Discovery continuation cancelled before Investigator admission."); }
      ...
      if (await shouldCancel()) { throw new Error("Discovery continuation cancelled before durable projection."); }
      await db.transaction(async (tx) => {
        const [locked] = await tx.select({ caseFile: researchCasesTable.caseFile, caseType: researchCasesTable.caseType }).from(researchCasesTable).where(eq(researchCasesTable.id, caseId)).for("update").limit(1);
        ...
        const observationKey = `discovery-continuation:case:${caseId}:job:${jobId}:turn:${iteration}:observation`;
        const observationPayload = JSON.stringify({ jobId, caseId, investigatorLlm: boss.investigatorLlm, searches: discovery.searches, visits: discovery.visits, status: discovery.status });
        const [insertedObservation] = await tx.insert(researchCaseEventsTable).values({ caseId, iteration, actorRole: "specialist", eventType: "observation", status: "recorded", summary: `Canonical continuation completed with ${discovery.findings.length} source-backed finding(s); no deterministic research lane executed.`, correlationKey: observationKey, payload: observationPayload }).onConflictDoNothing({ target: [researchCaseEventsTable.caseId, researchCaseEventsTable.correlationKey] }).returning({ id: researchCaseEventsTable.id });
        if (!insertedObservation?.id) {
          const [existingObservation] = await tx.select({ id: researchCaseEventsTable.id, payload: researchCaseEventsTable.payload, eventType: researchCaseEventsTable.eventType, actorRole: researchCaseEventsTable.actorRole }).from(researchCaseEventsTable).where(and(eq(researchCaseEventsTable.caseId, caseId), eq(researchCaseEventsTable.correlationKey, observationKey))).limit(1);
          if (!existingObservation?.id || existingObservation.eventType !== "observation" || existingObservation.actorRole !== "specialist" || existingObservation.payload !== observationPayload) throw new Error(`Discovery continuation observation replay mismatch for ${observationKey}`);
        }
      }, { isolationLevel: "serializable" });
    ...
    } catch (error) { const message = error instanceof Error ? error.message : "Canonical case continuation failed."; const cancelled = Boolean(await shouldCancel().catch(() => false)); await db.update(researchCasesTable).set({ status: "review", currentAction: cancelled ? "canonical-continuation-cancelled" : "canonical-continuation-error", updatedAt: new Date() }).where(eq(researchCasesTable.id, caseId)); await updateJob(jobId, { status: cancelled ? "cancelled" : "failed", outcome: "incomplete", message, finishedAt: new Date().toISOString() }); }
