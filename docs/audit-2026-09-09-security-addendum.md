...
This closes the specific validation-to-connect DNS re-resolution window identified during the audit. OWASP explicitly warns that validating a domain and then allowing the HTTP client to resolve it again creates a DNS-pinning/rebinding bypass. citeturn1search0turn1search13
...
The application mounts this middleware before the canonical route tree. Express middleware ordering makes that placement an actual request boundary. citeturn0search1
