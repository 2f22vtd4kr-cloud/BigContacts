...
18. Canonical target continuation is now model-owned: after each Investigator pass, DeepSeek provides oversight and Gemini chooses `continue_target`, `revisit_target`, `pivot_target`, or `stop`; the decision is durably persisted and subsequent passes remount the accumulated case context.
...
20. `scripts/audit-target-continuation-intrinsic.mjs` now checks the target runner's direct control wiring, Investigator-before-control ordering, explicit pass loop, stop-gated final review, and durable control persistence.
...
- **#139:** canonical target/Bureau cancellation state is fixed, but Python OSINT subprocesses (Holehe/Maigret/Sherlock/theHarvester) still lack real child-process cancellation tied to the run-scoped AbortSignal and formally governed network egress.
- **#147:** canonical ReAct still exposes compound `footprint_username`, `footprint_email`, and `harvest_domain` actions. A single model action deterministically fans into bundled Python tools. Required remediation is individually model-selectable capabilities plus cancellation/egress controls.
