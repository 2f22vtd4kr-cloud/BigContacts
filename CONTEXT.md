...
**Branch:** `main`  
**Tip at handoff write:** `PENDING` (confirm with `git log -1 --oneline` after pull) — gold-standard recovery: Atlas secondary+org surface, cookedAt boundary, 555 trash, collision labels, EDGAR co-filers, Groq-429 deterministic fallback, registry-first bounded jobs, related-person rank+display  
